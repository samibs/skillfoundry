...
- **Security hardening (audit remediation)** - Specter no longer executes model-authored shell commands; permission `ask` fails closed in headless runs; grep/semgrep de-shelled; per-project memory isolation; sanitizer catches JSON secrets + strips usernames; PRD/T4/SAST/security-micro gates fail closed; AgentPool hung-task deadlock fixed
...
| 5.32.0 | 2026-09-04 | AI Delivery Efficiency: deterministic LOW/MEDIUM/HIGH delivery budgets, risk-based execution policy, evidence reuse keyed to file content, validation deduplication, measured change impact, scoped test selection, objective stop conditions |
| 5.31.0 | 2026-09-04 | Governed Development Mission Protocol: durable `.ai/` ledger (execution + governance planes), worker attestation, stable patch-id provenance, remote-verified publication, collision-aware wave planning, ownership-verified process cleanup; AgentOS `enforce` contract mode |
| 5.30.0 | 2026-07-17 | AgentOS follow-ups: per-story state-slice streaming, flag-gated bus contract enforcement (off/permissive/strict), strict per-agent output contracts via archetype + advisory runtime check |
