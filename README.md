...
| | **Standalone CLI** (`sf`) | **IDE Skills** (103 skills) |
|---|---|---|
...
| **Full pipeline** | `sf forge`, `sf plan`, `sf gates` (23 commands) | `/forge`, `/go`, `/goma` (all 103 skills) |
...
103 skills install directly into your AI coding tool. This is the full SkillFoundry experience — all agents, all orchestration, autonomous mode, everything.
...
A separate terminal app with its own AI connection. Useful for provider switching, budget controls, and working outside an IDE. Has 23 native commands (not all 103 skills).
...
|----------|-------------------|---------------|-------|
| **Claude Code** | `.claude/commands/` (103 skills) | `/command` | Slash commands in Claude Code CLI |
...
| **OpenAI Codex** | `.agents/skills/` (103 skills) | `$command` | Dollar-prefix commands in Codex CLI |
| **Google Gemini** | `.gemini/skills/` (103 skills) | Skill invocation | Available in Gemini sessions |
...
├── .claude/commands/        Claude Code (103 skills)
├── .copilot/custom-agents/  GitHub Copilot (108 agents)
├── .cursor/rules/           Cursor (108 rules)
├── .agents/skills/          OpenAI Codex (103 skills)
└── .gemini/skills/          Google Gemini (103 skills)
```
