...
| | **Standalone CLI** (`sf`) | **IDE Skills** (105 skills) |
|---|---|---|
...
| **Full pipeline** | `sf forge`, `sf plan`, `sf gates` (23 commands) | `/forge`, `/go`, `/goma` (all 105 skills) |
...
105 skills install directly into your AI coding tool. This is the full SkillFoundry experience — all agents, all orchestration, autonomous mode, everything.
...
A separate terminal app with its own AI connection. Useful for provider switching, budget controls, and working outside an IDE. Has 23 native commands (not all 105 skills).
...
|----------|-------------------|---------------|-------|
| **Claude Code** | `.claude/commands/` (105 skills) | `/command` | Slash commands in Claude Code CLI |
...
| **OpenAI Codex** | `.agents/skills/` (105 skills) | `$command` | Dollar-prefix commands in Codex CLI |
| **Google Gemini** | `.gemini/skills/` (105 skills) | Skill invocation | Available in Gemini sessions |
...
├── .claude/commands/        Claude Code (105 skills)
├── .copilot/custom-agents/  GitHub Copilot (108 agents)
├── .cursor/rules/           Cursor (108 rules)
├── .agents/skills/          OpenAI Codex (105 skills)
└── .gemini/skills/          Google Gemini (105 skills)
```
