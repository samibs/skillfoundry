...
- **Skill/agent counts reconciled to verifiable ground truth.** The docs stated the skill count six different ways (63/64/65/60/66 across README, doc mastheads, and the site) and the agent count four ways (53/54/56/60) — none matching disk. Anchored every count to reproducible sources: **107 skills** per platform (what `install.sh` reports) and **61 core agents** (the framework's own `AGENT_REGISTRY`, whose `agent-registry.test.ts` asserts `toHaveLength(61)`). Propagated to the site changelog metrics (`54/66/64/60/60/61` → `61/107×5`), homepage stats, README (Supported Platforms table, IDE Skills header + tree, "…and 50 more" → "…and 92 more"), `docs/AGENTS.md`, `docs/QUICK-REFERENCE.md`, `docs/DOCUMENTATION-INDEX.md`, `site-docs/docs/architecture.md` (`88` → `112` agent prompt files), and `INDEX-v2` (honest breakdown: 61 registry · 56 persona files · 52 slash-command skills). Also corrected the stale `60` in the registry's own source comments (`// Registry: all 60 agents` → 61; `FULL (21 agents)` → 23, the actual per-category tally) and in `docs/TEST-SUITE-REFERENCE.md`, which described the test as expecting 60 when it asserts 61.
- **`sf_cli` dist rebuilt to match source.** The committed `dist/` was compiled from source predating the JSDoc doc-comments later added to `core/{agent,gates,pipeline}.ts`, and lagged the agent-registry `61` comment fix. A clean `tsc` rebuild produced a diff that is **entirely comment/JSDoc additions** — no functional or behavioral code changed (verified: every added `dist` line is a comment), and the build is idempotent. Brings tracked `dist` back in sync with tracked `src`. (The per-agent tool optimization was never at risk — it resolves via each agent's `toolCategory` + `getAgentTools()`, not a stored `tools` field.)
...
- **Item 2/10 — `/gosm`, `/goma`, `/blitz` collapsed to thin `/go` aliases.** The three go-mode wrappers each re-documented ~400 lines of pipeline ceremony that already lives in `/go`. They are now thin stubs that state their exact dispatch (`/gosm`→`/go --mode=semi-auto`, `/goma`→`/go --mode=autonomous`, `/blitz`→`/go --mode=semi-auto --parallel --tdd`) plus the one differentiator each genuinely adds — gosm's auto-fix-vs-escalate matrix, goma's clean-tree rollback precondition, blitz's ≥2-independent-story readiness + wave planning. `gosm.md` 398→43, `goma.md` 462→35, `blitz.md` 441→29 lines; regenerated consistently across all six platforms. Behavior preserved — the resolved `/go` invocation and each differentiator are unchanged; only duplicated pipeline documentation was removed.

- **Item 8/10 — `go.md` version-changelog framing removed from the operating prompt.** The `/go` skill carried four `## NEW IN v1.1.0 / v1.3.0 / v1.3.1 / v1.7.0` sections — a changelog living inside a live prompt, violating the framework's own README-vs-CHANGELOG separation. The live behavior was re-homed under capability headers (`## EXECUTION MODES & AUTO-REMEDIATION`, `## STATE, RECOVERY & METRICS`, `## SECURITY VALIDATION`) instead of version headers; the entire `NEW IN v1.3.1` section was deleted as a verbatim duplicate of Phase 3 (TDD Enforcement, Parallel Dispatch, Git Worktree Isolation, Systematic Debugging all already exist there); the stale "Dual-platform: Claude Code and GitHub Copilot" claim (now 6 platforms) and a marketing "Benefits" block were dropped. Regenerated across all six platforms. Behavior preserved — every live capability remains, now under a capability name rather than a version banner; version history lives in this CHANGELOG where it belongs.
- **Item 6/10 — Peer/Context ceremony boilerplate collapsed.** Two inert scaffolding blocks that no mechanism consumes — `Peer Improvement Signals` (upstream/downstream/challenge lists) and `Continuous Improvement Contract` — are removed from every source file and mirror, and the `Context Discipline` block (which referenced `_context-discipline.md` and then restated a quick-ref) is reduced to just its one-line reference. ~55 files across all six platform trees; ~600+ lines of repeated ceremony removed. Behavior preserved — the removed blocks drove no routing or gate; a precise model reads the referenced `_context-discipline.md`/`_reflection-protocol.md` canon.
- **Item 5/10 — Anvil tiers moved to a distinct A-namespace, ending the T-number collision.** Two gate systems shared the `T#` numbering with different meanings — the CLI quality gates (`sf_cli/core/gates.ts`, T0–T7: Type/Tests/Security/Build/…) and the Anvil agent-handoff checks (`agents/_anvil-protocol.md`, T0–T6: Canary/Self-Adversarial/Shadow/…). "T3 FAIL" meant "Tests" from the CLI but "Self-Adversarial Review" from `/go`/`/forge`/`/anvil`. The Anvil tiers are now **A0–A6** across every Anvil-definition and referencing file (7 definition files + `anvil`/`go`/`forge`/`stories`/`version`/`_known-deviations`), with a canonical namespace glossary in `_anvil-protocol.md` and a pointer note in `anvil`. The CLI T0–T7 system (`gates.ts`, `scripts/anvil.sh`, VS Code gate timeline) is unchanged. Behavior preserved — no check changed; only the Anvil label namespace was disambiguated so a gate number is no longer mode-dependent.
- **Item 10/10 — VS Code provider picker now shows its default explicitly.** The setup wizard's provider QuickPick gave no visible default, unlike the CLI wizard (which defaults to `[1]` = Anthropic). Anthropic (`PROVIDERS[0]`) is already the pre-highlighted first item, so it now carries a "Default — press Enter to accept" detail line and the placeholder says so — the common choice is one keystroke. Behavior preserved (all providers still selectable); the option `description` remains the provider id so the downstream provider lookup is unchanged.

### Fixed

- **Sync engine — standalone commands now generate Copilot & Cursor mirrors.** `scripts/sync-platforms.sh` generated standalone commands (those without a backing agent, e.g. `health`, `go`, `anvil`) only to Codex and Gemini — Copilot and Cursor were left as orphaned, hand-maintained files that silently drifted (which is why items 1/2/5/6/8 required manual Copilot/Cursor edits). Added `generate_copilot_standalone_content` / `generate_cursor_standalone_content` and wired them into the standalone loop, so all six platforms are now generated from the canonical `.claude/commands/` source. Regenerated 45 existing standalone mirrors per platform and created 10 previously-missing ones; sync integrity 52/52. Future standalone edits now propagate everywhere via `sync --all` — no hand-editing.

- **Item 9/10 — Platform count reconciled to 6 (Hexa) across docs.** Several current-state docs still said 5/Penta or listed fewer. The worst: `site-docs/docs/configuration.md` documented the `installed` config and `--platform` values as only `claude, copilot, cursor` — making Codex/Gemini/Grok **unreachable from the documented config**; now lists all 6. Also fixed: `site-docs/docs/architecture.md` ("5 AI coding platforms" → 6, noting Grok Build reuses the Codex format — 6 install platforms across 5 config formats), `.github/copilot-instructions.md` and `CONTRIBUTING.md` ("all 5 platforms" → 6), and `docs/DOCUMENTATION-INDEX.md` (Penta→Hexa + two stale version mastheads `v2.0.78`/`2.0.52`→`5.21.0`). Historical release-note platform counts (e.g. v5.10.0, pre-Grok) left as correct history. Docs only.
