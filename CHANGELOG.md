...
- **Skill/agent counts reconciled to verifiable ground truth.** The docs stated the skill count six different ways (63/64/65/60/66 across README, doc mastheads, and the site) and the agent count four ways (53/54/56/60) — none matching disk. Anchored every count to reproducible sources: **107 skills** per platform (what `install.sh` reports) and **60 core agents** (the framework's own `AGENT_REGISTRY`, enforced by `docs/TEST-SUITE-REFERENCE.md`). Propagated to the site changelog metrics (`54/66/64/60/60/61` → `60/107×5`), homepage stats, README (Supported Platforms table, IDE Skills header + tree, "…and 50 more" → "…and 92 more"), `docs/AGENTS.md`, `docs/QUICK-REFERENCE.md`, `docs/DOCUMENTATION-INDEX.md`, `site-docs/docs/architecture.md` (`88` → `112` agent prompt files), and `INDEX-v2` (now with an honest breakdown: 60 registry · 56 persona files · 52 slash-command skills). Docs only.
