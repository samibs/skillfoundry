...
# Workspace config (keep config.toml tracked, ignore runtime data)
.skillfoundry/
...
WRAPPER_EOF
    ...
    # ─────────────────────────────────────────────────────────────
    # Ensure .skillfoundry/config.toml exists (enables VS Code extension)
    # ─────────────────────────────────────────────────────────────
    if [ ! -f "$project_dir/.skillfoundry/config.toml" ]; then
        mkdir -p "$project_dir/.skillfoundry"
        local platforms_toml
        platforms_toml=$(echo "$platforms" | tr ' ' '\n' | sed 's/.*/"&"/' | paste -sd, - | sed 's/,/, /g')
        cat > "$project_dir/.skillfoundry/config.toml" << TOML_EOF
# SkillFoundry Configuration
# Generated by update.sh v${FRAMEWORK_VERSION}
...
log_level = "info"
TOML_EOF
        echo -e "  ${GREEN}✓ Created .skillfoundry/config.toml (enables VS Code extension)${NC}"
    else
        # Update version in existing config
        sed -i "s/^version = .*/version = \"${FRAMEWORK_VERSION}\"/" "$project_dir/.skillfoundry/config.toml" 2>/dev/null || true
    fi
