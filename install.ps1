...
New-Item -ItemType Directory -Force -Path (Join-Path $TargetDir ".skillfoundry") | Out-Null

# Create config.toml (enables VS Code extension activation + framework config)
$configToml = Join-Path $TargetDir ".skillfoundry\config.toml"
if (-not (Test-Path $configToml)) {
    $platformsList = ($Platforms | ForEach-Object { "`"$_`"" }) -join ", "
    $tomlContent = @"
# SkillFoundry Configuration
# Generated by install.ps1 v$Version
...
"@
    ...
    Write-ColorOutput "  [OK] .skillfoundry/config.toml created" "Green"
}
