---
description: Full per-feature pipeline: implement → testloop → evaluator challenge → coder feedback → document → commit.
globs:
alwaysApply: false
---
...
> **Activation**: Say "feature" or "use feature rule" in chat to activate this workflow.
...
```
When the user says "feature" with                            Prompt for story or description
When the user says "feature" with STORY-001                  Run lifecycle for a specific story file
When the user says "feature" with "description of feature"   Run lifecycle for an inline description
When the user says "feature" with --dry-run                  Preview pipeline without executing
When the user says "feature" with --from challenge           Resume from a specific stage
When the user says "feature" with --no-commit                Run full pipeline but don't commit
When the user says "feature" with --override "reason"        Accept current gate failure with logged tradeoff
When the user says "feature" with --exec-mode advisory       Force advisory mode (no shell-based test claims)
When the user says "feature" with --detect-stack             Re-run stack detection before starting
When the user says "feature" with --lite                     Lite mode: no shadow tester, no Anvil, single-pass evaluator
```
...
```
When the user says "feature" with --from testloop     Resume from Stage 2
When the user says "feature" with --from challenge    Resume from Stage 3
When the user says "feature" with --from document     Resume from Stage 4
When the user says "feature" with --from commit       Resume from Stage 5
```
...
- "use feature rule"
- "feature — implement the authentication feature"
- "follow the feature workflow for this task"
