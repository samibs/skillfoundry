---
description: Use this agent to find and safely remove dead code and duplicate code — unused imports, unused exports, and copy-paste blocks — in any codebase.
globs:
alwaysApply: false
---
...
> **Activation**: Say "prune" or "use prune rule" in chat to activate this workflow.
...
```
When the user says "prune" with                      Scan the current project (duplicates + dead code), report
When the user says "prune" with scan [--path <dir>] [--deep]   Full scan; --deep also runs jscpd/knip/depcheck if present
When the user says "prune" with duplicates [--min-lines N]     Copy-paste blocks only (report — extraction is a refactor)
When the user says "prune" with deadcode [--fix]               Unused imports/exports; --fix removes the SAFE subset only
```
...
- "use prune rule"
- "prune — implement the authentication feature"
- "follow the prune workflow for this task"
