---
description: Implementation-test-fix feedback loop.
globs:
alwaysApply: false
---

# testloop — Cursor Rule

> **Activation**: Say "testloop" or "use testloop rule" in chat to activate this workflow.
...
```
When the user says "testloop" with                          Auto-detect scope and run all tests
When the user says "testloop" with unit                     Unit tests only
When the user says "testloop" with e2e                      Playwright E2E tests only
When the user says "testloop" with all                      All layers: unit + integration + E2E
When the user says "testloop" with --max 3                  Override max iterations (default: 5)
When the user says "testloop" with --file path/to/test.ts   Run a specific test file only
When the user says "testloop" with --story STORY-001        Run tests tagged to a story
When the user says "testloop" with --exec-mode advisory     Force advisory mode (planning, no shell)
```
...
- "use testloop rule"
- "testloop — implement the authentication feature"
- "follow the testloop workflow for this task"
