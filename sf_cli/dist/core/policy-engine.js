...
// ── Policy checks ─────────────────────────────────────────────────────────────
function checkApprovedModels(config, teamConfig, violations) {
    if (!teamConfig.approved_models || teamConfig.approved_models.length === 0)
        return;
    const isApproved = teamConfig.approved_models.some((m) => config.model === m || config.model.startsWith(m));
    if (!isApproved) {
        violations.push({
...
function checkMemorySync(config, teamConfig, violations) {
    if (!teamConfig.memory?.auto_sync)
        return;
...
function checkBannedPatterns(teamConfig, violations) {
    if (!teamConfig.gates?.t1_mode)
        return;
...
function checkSkillVersion(teamConfig, currentVersion, violations) {
    if (!teamConfig.skills?.version)
        return;
 ...
 */
export function checkPolicy(workDir, currentVersion = '0.0.0') {
    const log = getLogger();
