...
async function validatePrdQuality(prds, model, genesisDir) {
    ...
    for (const prd of prds) {
        ...
        try {
            ...
            throw new PrdQualityBlockError(`PRD gate could not evaluate ${prd.file}: ${errMessage}\n` +
...
export async function runPipeline(options) {
    ...
    updatePhase('POLISH', 'passed', Date.now() - polishStart, `${polishFindings.length} reviews, ${polishFails} issues${hasMgFailures ? ' (fixer applied)' : ''}`);
