...
export function classifyProviderError(err) {
    ...
    if (/insufficient.*quota|billing|payment.*required|rate.*limit|429/i.test(message)) {
        return (`Quota/billing error: ${message}\n\n` +
...
export async function runAgentLoop(messages, options, callbacks) {
    ...
    let budgetCache = loadUsage(workDir);
    // Pre-flight budget check
    const budgetCheck = checkBudget(workDir, config.monthly_budget_usd, config.run_budget_usd, 0, budgetCache ?? undefined);
    ...
    while (turnCount < maxTurns) {
        ...
        const turnBudget = checkBudget(workDir, config.monthly_budget_usd, config.run_budget_usd, totalCostUsd, budgetCache ?? undefined);
        ...
        try {
            toolStreamResult = await streamWithRetry(async (p) => p.streamWithTools(messagesForTurn, {
                model: config.model,
                tools,
        ...
        const assistantContent = result.content.map((block) => {
            if (block.type === 'text') {
