...
function walkFiles(dir, maxDepth = 4, depth = 0) {
...
function scanForPatterns(files, patterns, skipTest = true) {
    const findings = [];
    for (const file of files) {
        const rel = basename(file);
        // Skip test files
        if (skipTest && (/\.test\.|\.spec\.|__tests__|test[/\\]/i.test(file)))
            continue;
        // Skip build artifacts, scripts, and generated content
        if (/node_modules|dist[/\\]|build[/\\]|\.min\.|scripts[/\\]|data[/\\]/i.test(file))
            continue;
        // Skip files that define patterns (not use them)
        if (SECURITY_SCAN_EXCLUDE_FILES.has(rel))
            continue;
        const ext = extname(file).toLowerCase();
        if (!['.ts', '.tsx', '.js', '.jsx', '.py', '.cs', '.java', '.go', '.rb', '.php', '.html', '.vue', '.svelte'].includes(ext))
            continue;
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (const pat of patterns) {
                ...
                if (/new RegExp\(|\/.*\/[gimsuy]*/.test(trimmed) && /pattern|regex|match|detect|scan/i.test(trimmed))
                    continue;
                findings.push({
                    severity: 'high',
                    category: 'security',
...
// ── Category Analyzers ──────────────────────────────────────────
export function auditSecurity(projectPath) {
...
export function auditDocumentation(projectPath) {
    const start = Date.now();
    const findings = [];
    const docs = [
        { file: 'README.md', severity: 'critical', title: 'No README.md' },
        { file: 'CHANGELOG.md', severity: 'medium', title: 'No CHANGELOG.md' },
        { file: 'LICENSE', severity: 'high', title: 'No LICENSE file' },
    ];
    for (const d of docs) {
...
export function auditTesting(projectPath) {
    const start = Date.now();
    const findings = [];
    const files = walkFiles(projectPath);
...
export function auditDependencies(projectPath) {
    const start = Date.now();
    const findings = [];
...
export function auditLicense(projectPath) {
    const start = Date.now();
    const findings = [];
...
export function auditAccessibility(projectPath) {
    const start = Date.now();
    const findings = [];
    const files = walkFiles(projectPath);
...
export function auditPrivacy(projectPath) {
    const start = Date.now();
    const findings = [];
    const files = walkFiles(projectPath);
...
export function auditArchitecture(projectPath) {
    const start = Date.now();
    const findings = [];
    const files = walkFiles(projectPath);
...
export function auditSeo(projectPath) {
    const start = Date.now();
    const findings = [];
    const files = walkFiles(projectPath);
...
export function auditPerformance(projectPath) {
    const start = Date.now();
    const findings = [];
    const files = walkFiles(projectPath);
...
export function auditCiCd(projectPath) {
    const start = Date.now();
    const findings = [];
...
export function auditContracts(projectPath) {
    const start = Date.now();
    const files = walkFiles(projectPath);
    const findings = [];
    const frontendFiles = files.filter((f) => ['.tsx', '.jsx', '.ts', '.js', '.vue', '.svelte'].includes(extname(f).toLowerCase()) &&
        !/\.test\.|\.spec\.|__tests__|node_modules/i.test(f));
    const backendFiles = files.filter((f) => ['.ts', '.js', '.py', '.cs', '.java', '.go', '.rb', '.php'].includes(extname(f).toLowerCase()) &&
        !/\.test\.|\.spec\.|__tests__|node_modules/i.test(f) &&
...
export function auditAuthorization(projectPath) {
    const start = Date.now();
    const files = walkFiles(projectPath);
    const findings = [];
    const routeFiles = files.filter((f) => ['.ts', '.js', '.py', '.cs', '.java', '.go'].includes(extname(f).toLowerCase()) &&
        !/\.test\.|\.spec\.|__tests__|node_modules/i.test(f) &&
...
export function auditErrorHandling(projectPath) {
    const start = Date.now();
    const files = walkFiles(projectPath);
    const findings = [];
    const srcFiles = files.filter((f) => ['.ts', '.tsx', '.js', '.jsx', '.py', '.cs', '.java', '.go'].includes(extname(f).toLowerCase()) &&
        !/\.test\.|\.spec\.|__tests__|node_modules/i.test(f) &&
        !SECURITY_SCAN_EXCLUDE_FILES.has(basename(f)));
    for (const file of srcFiles.slice(0, 100)) {
        const content = readSafe(file);
...
export function auditSupplyChain(projectPath) {
    const start = Date.now();
    const findings = [];
...
// ── DB Persistence ──────────────────────────────────────────────
export function insertCertificationRun(db, result) {
    db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(result.id, result.projectPath, result.projectName, result.grade, result.overallScore, result.totalFindings, result.findingsBySeverity.critical || 0, result.findingsBySeverity.high || 0, result.findingsBySeverity.medium || 0, result.findingsBySeverity.low || 0, result.durationMs, result.startedAt, result.completedAt, JSON.stringify(result.categories.map((c) => ({ category: c.category, score: c.score, pass: c.pass }))));
...
export function getCertificationRun(db, runId) {
    const row = db.prepare('SELECT * FROM certification_runs WHERE id = ?').get(runId);
    if (!row)
        return null;
    const findings = db.prepare('SELECT * FROM certification_findings WHERE run_id = ?').all(runId);
    return {
        id: row.id,
...
export function getCertificationHistory(db, limit = 20) {
    return db.prepare('SELECT id, project_name, grade, overall_score, total_findings, completed_at FROM certification_runs ORDER BY completed_at DESC LIMIT ?').all(limit);
}
// ── Formatting ──────────────────────────────────────────────────
const GRADE_COLOR = { A: '\x1b[32m', B: '\x1b[36m', C: '\x1b[33m', D: '\x1b[33m', F: '\x1b[31m' };
const NC = '\x1b[0m';
export function formatCertificationReport(result) {
    const lines = [];
