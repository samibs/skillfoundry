...
// ---------------------------------------------------------------------------
let _commandIndex = null;
function buildCommandIndex() {
    const index = new Map();
    try {
        const root = getFrameworkRoot();
        const agentsDir = join(root, 'agents');
        const files = readdirSync(agentsDir).filter((f) => f.endsWith('.md') && !f.startsWith('_') && !f.startsWith('INDEX') && !f.startsWith('UPGRADE'));
 ...
 */
export function loadAgentPromptFromFile(agentName) {
 ...
 */
export function _resetPromptLoaderCache() {
    _commandIndex = null;
    _promptCache.clear();
}
