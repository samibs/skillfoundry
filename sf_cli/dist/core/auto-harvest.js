 ...
 */
export function harvestStoryCompletion(event) {
    const log = getLogger();
    const entries = [];
    // Files created → fact
 ...
 */
export function harvestGateFailure(event) {
    const log = getLogger();
 ...
 */
export function harvestFixerIntervention(event) {
    const log = getLogger();
 ...
 */
export function harvestSessionSummary(event) {
    const log = getLogger();
    const entry = {
        type: 'fact',
...
function extractTags(filePaths) {
    ...
    for (const fp of filePaths) {
        ...
        for (const part of parts) {
            if (['src', 'dist', 'node_modules', '__tests__', 'test', 'tests'].includes(part))
                continue;
            if (part.includes('.'))
...
//# sourceMappingURL=auto-harvest.js.map