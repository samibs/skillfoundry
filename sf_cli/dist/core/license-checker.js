 ...
 */
export function detectProjectType(targetPath) {
    ...
    if (existsSync(sfrc)) {
        try {
            ...
            if (parsed.projectType === 'open-source')
                return 'open-source';
            if (parsed.projectType === 'commercial')
                return 'commercial';
        }
        catch {
    ...
    if (existsSync(sfToml)) {
        try {
            ...
            if (policyMatch) {
                ...
                if (policy === 'open-source' || policy === 'opensource')
                    return 'open-source';
 ...
 */
export function evaluateLicense(normalised, projectType, flagLicenses, allowLicenses) {
    ...
    if (allowLicenses.includes(normalised))
        return null;
    // Permissive licenses are always safe
    if (PERMISSIVE_LICENSES.has(normalised))
        return null;
    // Open-source projects: only flag AGPL
    if (projectType === 'open-source') {
        if (normalised.startsWith('AGPL') &&
 ...
 */
export function scanNpmLicenses(targetPath, manifestPath) {
    const packages = [];
    let pkgJson;
    try {
    ...
    for (const [name, reqVersion] of Object.entries(allDeps)) {
        ...
        if (typeof reqVersion !== 'string' || reqVersion.startsWith('workspace:'))
            continue;
 ...
 * @returns Array of packages with their resolved licenses.
 */
export function scanPipLicenses(targetPath, manifestPath) {
    const packages = [];
    let content;
    try {
 ...
 */
function findPythonSitePackages(targetPath) {
    const candidates = [];
    ...
    for (const venv of venvNames) {
        ...
        if (!existsSync(libPath))
            continue;
        try {
            const libEntries = readdirSync(libPath, { withFileTypes: true, encoding: 'utf8' });
 ...
 */
function readPipDistInfoLicense(sitePackagesDir, safeName) {
    try {
        const entries = readdirSync(sitePackagesDir, { withFileTypes: true, encoding: 'utf8' });
 ...
 * @returns Array of packages with their resolved licenses.
 */
export function scanCargoLicenses(_targetPath, manifestPath) {
    const packages = [];
    let content;
    try {
        content = readFileSync(manifestPath, 'utf-8');
 ...
 */
export function scanGoLicenses(_targetPath, manifestPath) {
    const packages = [];
    let content;
    try {
        content = readFileSync(manifestPath, 'utf-8');
 ...
 */
export function scanDotnetLicenses(_targetPath, manifestPath) {
    const packages = [];
    let content;
    try {
        content = readFileSync(manifestPath, 'utf-8');
 ...
 */
export function discoverManifests(targetPath) {
    const manifests = [];
 ...
 */
export function createLicenseChecker(projectRoot) {
