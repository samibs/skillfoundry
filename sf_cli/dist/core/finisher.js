...
export function readCanonicalVersion(workDir) {
...
export function checkVersionConsistency(workDir, expectedVersion) {
    const results = [];
...
export function fixVersionReferences(workDir, oldVersion, newVersion) {
    const results = [];
...
export function runVersionCheck(workDir, mode, storiesCompleted) {
...
export function getActualTestCount(workDir) {
    try {
...
export function checkTestCounts(workDir, actualCount) {
    const drifts = [];
...
export function fixTestCounts(workDir, oldCount, newCount) {
    const results = [];
    ...
    for (const file of files) {
        ...
        const updated = original.replace(new RegExp(`\\b${oldCount}(\\+?\\s*tests?)`, 'gi'), `${newCount}$1`).replace(new RegExp(`Tests \\(${oldCount} tests`, 'g'), `Tests (${newCount} tests`).replace(new RegExp(`Tests\\s*\\|\\s*${oldCount}`, 'g'), `Tests | ${newCount}`).replace(new RegExp(`all ${oldCount} TypeScript`, 'gi'), `all ${newCount} TypeScript`).replace(new RegExp(`Run all ${oldCount} tests`, 'g'), `Run all ${newCount} tests`);
        if (updated !== original) {
            writeFileSync(filePath, updated, 'utf-8');
            results.push({ file, updated: true });
        }
...
export function runTestCountCheck(workDir, mode) {
    const start = Date.now();
...
export function scanCoreModules(workDir) {
...
export function extractDocArchListing(workDir) {
...
export function diffArchListing(onDisk, inDocs) {
...
export function runArchitectureCheck(workDir, _mode) {
    const start = Date.now();
    ...
    if (diff.missing.length > 0)
        parts.push(`missing from docs: ${diff.missing.join(', ')}`);
    if (diff.extra.length > 0)
        parts.push(`in docs but not on disk: ${diff.extra.join(', ')}`);
    return {
        check: 'architecture',
        status: 'drift',
        detail: parts.join('; '),
...
export function checkChangelogEntry(workDir, version) {
...
export function runChangelogCheck(workDir, mode, version) {
    const start = Date.now();
    const filePath = join(workDir, 'CHANGELOG.md');
    if (!existsSync(filePath)) {
        return {
...
export function checkGitClean(workDir) {
    try {
...
export function runGitCleanCheck(workDir, _mode) {
    const start = Date.now();
 ...
 */
export async function runFinisher(options) {
...
//# sourceMappingURL=finisher.js.map