...
function reusedSeconds(reusedKeys, entries) {
    ...
    if (known.length === 0)
        return reusedKeys.length === 0 ? 0 : null;
    if (known.length < reusedKeys.length)
        return null;
...
export function buildEfficiencyReport(workDir, mission = 'default') {
    ...
    if (evidenceReuseRate !== null && evidenceReuseRate < 0.2 && denominator >= 5) {
        observations.push(`Evidence reuse is ${(evidenceReuseRate * 100).toFixed(0)}%. Either scoped file sets are too broad to survive edits, or agents are not consulting the store before running.`);
    }
    ...
    if (repoWideWorkers.length > 1) {
        observations.push(`${repoWideWorkers.length} workers each ran a repository-wide scope. That cost belongs at the integration gate, once.`);
    }
    const lowBudgetBroad = tasks.filter((t) => t.budget === 'LOW' && t.validationScope && REPO_WIDE_SCOPES.includes(t.validationScope));
    if (lowBudgetBroad.length > 0) {
        observations.push(`${lowBudgetBroad.length} LOW-budget task(s) ran a repository-wide scope. A LOW change should not trigger the full suite without escalation evidence.`);
    }
    if (tasks.length === 0) {
...
export function formatEfficiencyReport(report) {
    ...
    if (report.tasks.length > 0) {
        ...
        for (const task of report.tasks) {
            lines.push(`  ${task.taskId.padEnd(16)} ${(task.budget ?? '—').padEnd(7)} ${(task.validationScope ?? '—').padEnd(12)} ` +
...
//# sourceMappingURL=delivery-metrics.js.map