...
// ── Scoring prompt ─────────────────────────────────────────────────────────
const SCORING_SYSTEM_PROMPT = 'You are a rigorous technical PRD reviewer. You evaluate product requirements documents with cold objectivity — no score inflation, no flattery. Your assessments are used to gate implementation work, so inaccurate scores waste engineering effort.';
/**
 ...
 */
export function buildScoringPrompt(prdContent) {
...
export const SCORE_PRD_TOOL = {
    name: 'score_prd',
    description: 'Return a structured quality evaluation of the PRD across four dimensions: completeness, specificity, consistency, and scope.',
    input_schema: {
        type: 'object',
        properties: {
            completeness: {
...
function validateDimension(dim, name) {
    ...
    if (typeof score !== 'number' || !Number.isInteger(score) || score < 1 || score > 10) {
        throw new Error(`PRD scorer: dimension "${name}.score" must be an integer between 1 and 10, got ${JSON.stringify(score)}`);
    }
    const justification = d['justification'];
    if (typeof justification !== 'string' || justification.length < 20) {
        throw new Error(`PRD scorer: dimension "${name}.justification" must be a string of at least 20 characters`);
    }
 ...
 */
export function validateScoringResponse(raw) {
    ...
    if (!Array.isArray(rawSuggestions) || rawSuggestions.length < 1 || rawSuggestions.length > 10) {
        throw new Error(`PRD scorer: "suggestions" must be an array with 1-10 items, got ${JSON.stringify(rawSuggestions)}`);
    }
 ...
 */
export function isPrdContent(content) {
 ...
 */
function setCache(cacheKey, score) {
...
export class PrdNotDetectedError extends Error {
    constructor(prdPath) {
        super(`File does not appear to be a PRD (missing frontmatter or required sections): ${prdPath}`);
 ...
 */
function extractToolInput(contentBlocks) {
 ...
 */
async function callScoringLlm(provider, model, messages) {
 ...
 */
export async function scorePrd(prdContent, prdPath, provider, model) {
    ...
    try {
        ...
        try {
            ...
            throw new PrdScoringError(`PRD scoring failed after retry for "${prdPath}": ${retryErr instanceof Error ? retryErr.message : String(retryErr)}`, retryErr);
        }
    }
    // Build PrdScore
    const allScores = [
 ...
 * @throws PrdScoringError when LLM scoring fails after one retry.
 */
export async function scorePrdFile(prdPath, provider, model) {
 ...
 */
export async function scoreMultiplePrds(prdPaths, provider, model) {
    const logger = getLogger();
    const results = [];
    const errors = [];
    ...
    if (errors.length > 0) {
        throw new PrdScoringError(`PRD batch scoring completed with ${errors.length} failure(s):\n${errors.join('\n')}`);
    }
    return results;
}
...
export class PrdScorer {
     ...
     */
    async scoreMultiple(prdPaths) {
        return scoreMultiplePrds(prdPaths, this.provider, this.model);
    }
}
//# sourceMappingURL=prd-scorer.js.map