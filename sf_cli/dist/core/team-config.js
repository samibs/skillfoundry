 ...
 */
export function loadTeamConfig(workDir, configPath) {
    const log = getLogger();
    ...
    try {
        ...
        throw new TeamConfigError(`Failed to parse team config: ${err instanceof Error ? err.message : String(err)}`);
    }
 ...
 */
export function mergeGateThresholds(teamGates) {
    return {
 ...
 */
export function isModelApproved(model, teamConfig) {
    if (!teamConfig?.approved_models || teamConfig.approved_models.length === 0) {
        return true;
    }
    return teamConfig.approved_models.some((approved) => model === approved || model.startsWith(approved));
}
