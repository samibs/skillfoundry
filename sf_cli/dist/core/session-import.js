...
import { insertSessionReport, insertSessionIssues, insertFailurePatterns, } from './dashboard-db.js';
// ── Session Report Import ──────────────────────────────────────
/**
 * Import a session report JSON into the database.
 * Returns true if imported, false if duplicate.
 */
export function importSessionReport(db, projectId, report, sourceFile) {
    const log = getLogger();
    ...
    // Insert issues
    const issueRecords = report.issues.map((issue) => ({
        id: issue.id,
    ...
    const patternRecords = report.errorPatterns.map((ep) => ({
        project_id: projectId,
 ...
 */
export function importForgeLog(db, projectId, logPath) {
    const log = getLogger();
 ...
 */
export function scanProjectSessions(db, projectId, projectPath) {
    const result = {
        sessions_imported: 0,
        sessions_skipped: 0,
        issues_imported: 0,
        ...
        try {
            ...
            for (const file of files) {
                ...
                try {
                    ...
                    if (!report.runId || !report.summary)
                        continue;
 ...
 */
export function importInbox(db, frameworkDir, projectIdOverride) {
    const log = getLogger();
    ...
    let files;
    try {
        files = readdirSync(inboxDir).filter((f) => f.endsWith('.json') || f.endsWith('.log'));
    }
    catch {
        return result;
    }
        ...
        try {
            if (file.endsWith('-issues.json') || file.endsWith('.json')) {
                ...
                if (data.runId && data.summary) {
                    ...
                    const res = importSessionReport(db, projectId, data, filePath);
                    if (res.imported) {
                        result.sessions_imported++;
                        result.issues_imported += res.issues;
                        result.patterns_extracted += res.patterns;
...
// ── Helpers ──────────────────────────────────────────────────
function extractProjectIdFromReport(db, data) {
    // Check if report contains a project_id or project_path hint
    const projectPath = data.project_path;
    if (projectPath) {
        const row = db.prepare('SELECT id FROM projects WHERE path = ?').get(projectPath);
        if (row)
            return row.id;
    }
    return null;
}
function createUnknownProject(db) {
    const id = 'unknown-' + randomUUID().slice(0, 8);
    const existing = db.prepare("SELECT id FROM projects WHERE name = 'unknown-inbox'").get();
    if (existing)
        return existing.id;
    db.prepare(`
...
//# sourceMappingURL=session-import.js.map