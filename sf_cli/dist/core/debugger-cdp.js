    ...
    static create(url) {
        return new Promise((resolve, reject) => {
            ...
            req.on("upgrade", (_res, socket, head) => {
                ...
                socket.on("data", (chunk) => ws.onData(chunk));
...
export class CDPAdapter {
    ...
    pauseLocation = null;
    callFrames = [];
    scriptMap = new Map(); // scriptId → url
    connected = false;
    // -----------------------------------------------------------------------
    // Connection
    // -----------------------------------------------------------------------
    ...
    async setBreakpoint(file, line, condition) {
        ...
        return {
            ...
            file,
            line: verified ? locations[0].lineNumber : line,
            condition,
            verified,
        };
    }
    ...
    async getScope() {
        ...
        if (!topFrame)
            return [];
        const variables = [];
        const scopes = topFrame.scopeChain ?? [];
    ...
    formatVariablePreview(obj) {
        if (!obj)
            return undefined;
        if (obj.preview)
            return this.stringifyPreview(obj.preview);
        ...
        if (obj.description)
            return String(obj.description).slice(0, VALUE_PREVIEW_LIMIT);
        return undefined;
    }
    /**
     * Send a CDP method call and wait for the matching response.
    ...
    stringifyPreview(preview) {
        ...
        for (const p of preview.properties) {
            ...
            if (entries.join(", ").length > VALUE_PREVIEW_LIMIT)
                break;
        }
...
//# sourceMappingURL=debugger-cdp.js.map