 ...
 */
export function transformVarToConst(code) {
    let count = 0;
 ...
 */
export function transformAddTypes(code) {
    let count = 0;
    // Match function declarations with untyped params
    const transformed = code.replace(/function\s+\w+\s*\(([^)]*)\)/g, (match, params) => {
        if (!params.trim())
            return match;
 ...
 */
export function transformWrapAsync(code) {
 ...
 */
export function transformAddExport(code) {
    let count = 0;
    const transformed = code.replace(/^(function\s+\w+|class\s+\w+)/gm, (match) => {
        count++;
        return `export ${match}`;
    });
    // Don't double-export
...
export function transformRequireToImport(code) {
    ...
    transformed = transformed.replace(/const\s+(\w+)\s*=\s*require\s*\(\s*['"]([^'"]+)['"]\s*\)/g, (_match, name, mod) => {
        count++;
        return `import ${name} from '${mod}'`;
    });
    // const { A, B } = require('Y') → import { A, B } from 'Y'
    transformed = transformed.replace(/const\s*\{([^}]+)\}\s*=\s*require\s*\(\s*['"]([^'"]+)['"]\s*\)/g, (_match, names, mod) => {
        count++;
 ...
 */
export function transformAddJsdoc(code) {
    const lines = code.split('\n');
    const result = [];
    let count = 0;
    for (let i = 0; i < lines.length; i++) {
        ...
        if (funcMatch) {
            ...
            if (params.trim()) {
                for (const p of params.split(',')) {
                    ...
                    if (paramName)
                        jsdoc.push(`${indent} * @param ${paramName}`);
                }
            }
 ...
 */
export function detectApplicableTransforms(code) {
    const applicable = [];
    if (/\bvar\s+\w+/.test(code))
        applicable.push('var-to-const');
    ...
    if (/\bawait\b/.test(code))
        applicable.push('wrap-async');
    if (/^(?:async\s+)?function\s+\w+/m.test(code) && !/^export\s/m.test(code))
        applicable.push('add-export');
    if (/\brequire\s*\(/.test(code))
        applicable.push('require-to-import');
 ...
 */
export function boostCode(code, options) {
 ...
 */
export function boostFile(filePath, options) {
    const code = readFileSync(filePath, 'utf-8');
    return boostCode(code, options);
}
// ── Formatting ──────────────────────────────────────────────────
export function formatBoostReport(result) {
    const lines = [
        'Code Booster',
        LINE.repeat(60),
...
export function formatTransformList() {
...
//# sourceMappingURL=code-booster.js.map