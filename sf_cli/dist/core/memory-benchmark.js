 ...
 */
export function calculatePrecisionAtK(retrievedIds, expectedIds, topK) {
 ...
 */
export function loadBenchmarkDataset(datasetPath) {
 ...
 */
export function generateReport(queryResults, passThreshold, corpusSize) {
    ...
    const overallPrecision = queryResults.reduce((sum, r) => sum + r.precision, 0) / queryResults.length;
 ...
 */
export function compareToBaseline(currentPrecision, baseline) {
 ...
 */
export class VectorStoreBenchmarkAdapter {
    store;
    /**
     ...
     */
    async indexCorpus(corpus) {
        const documents = corpus.map((doc) => ({
            id: doc.id,
            text: doc.text,
 ...
 */
export class TfIdfBenchmarkAdapter {
    corpus = [];
    /**
    ...
    async search(query, topK) {
        ...
        const scored = this.corpus.map((doc) => {
            ...
            const tagsLower = (doc.metadata.tags ?? []).map((t) => t.toLowerCase());
 ...
 */
export async function runPrecisionBenchmark(dataset, adapter, baselinePath) {
    const log = getLogger();
 ...
 */
export function formatBenchmarkReport(report, baselineComparison) {
    const lines = [];
 ...
 */
export function defaultDatasetPath(fromDir) {
 ...
 */
export function defaultBaselinePath(fromDir) {
