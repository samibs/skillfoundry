...
export class TimeoutError extends Error {
    ...
    correlationId;
    constructor(correlationId, timeoutMs) {
...
export class AgentMessageBus {
     ...
     */
    subscribeOnce(topic, handler, agentId) {
     ...
     */
    static buildMessage(sender, recipient, type, payload, correlationId) {
        return {
            id: randomUUID(),
            sender,
            recipient,
    ...
    // Private helpers
    // ---------------------------------------------------------------------------
    addSubscriber(topic, handler, once, agentId) {
        const entry = {
            id: randomUUID(),
            topic,
            handler,
        ...
        return () => {
            ...
            this.subscribers.set(topic, current.filter((e) => e.id !== entry.id));
        };
    }
    /**
     * Execute the middleware chain. When all middleware have called `next()`, invoke `done`.
     */
    runMiddleware(message, index, done) {
    ...
    deliver(message) {
        ...
        for (const entry of candidates) {
            ...
            if (entry.once) {
                ...
                this.subscribers.set(entry.topic, current.filter((e) => e.id !== entry.id));
            }
            try {
