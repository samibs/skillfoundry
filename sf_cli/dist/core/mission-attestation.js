...
import { isGitRepo, topLevel, commonGitDir, currentBranch, revParse, treeSha, workingTreeStatus, isNativeWorktree, hasCommitIdentity, remoteUrl, } from './mission-git.js';
...
export function environmentFingerprint() {
    const ci = (process.env.GITHUB_ACTIONS && 'github') ||
...
export function attestationPath(workDir, missionId, worker) {
 ...
 */
export function attestWorker(workDir, opts) {
    assertMissionId(opts.missionId);
    if (!WORKER_NAME_PATTERN.test(opts.worker)) {
        throw new Error(`Invalid worker name "${opts.worker}": expected 1-48 chars of [A-Za-z0-9._-] starting alphanumeric`);
    }
    const blockers = [];
    const warnings = [];
    ...
    if (!worktreeCheck.registered) {
        blockers.push('This directory is not registered in `git worktree list --porcelain`. ' +
    ...
    if (isMainWorktree && !opts.allowMainWorktree) {
        blockers.push('Worker is attesting in the repository main checkout. Parallel workers must each own ' +
    ...
    if (!status.clean && !opts.allowDirty) {
        ...
        blockers.push(`Working tree is dirty (${n} product entr${n === 1 ? 'y' : 'ies'}): ${sample}` +
            `${n > 5 ? ' …' : ''}. A dirty tree is not a valid implementation baseline.`);
    }
    if (status.governanceEntries.length > 0) {
        warnings.push(`${status.governanceEntries.length} uncommitted governance artifact(s) under .ai/ or .skillfoundry/ — ` +
    ...
    const missingAssets = (opts.requiredAssets ?? []).filter((a) => !existsSync(resolve(resolvedWorkDir, a)));
...
export function readAttestation(workDir, missionId, worker) {
...
export function listAttestations(workDir, missionId) {
    assertMissionId(missionId);
 ...
 */
export function gateOnAttestation(workDir, missionId, worker) {
    const attestations = worker
        ? [readAttestation(workDir, missionId, worker)].filter((a) => a !== null)
 ...
 */
export function attestationDrift(workDir, attestation) {
    const drift = [];
    const resolvedWorkDir = resolve(workDir);
    if (resolve(attestation.worktree_path) !== resolvedWorkDir) {
        drift.push(`Worker attested in ${attestation.worktree_path} but is now operating in ${resolvedWorkDir}`);
    }
...
export function relativeAttestationPath(workDir, missionId, worker) {
    return relative(resolve(workDir), attestationPath(workDir, missionId, worker));
}
//# sourceMappingURL=mission-attestation.js.map