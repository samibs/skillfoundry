 ...
 */
export function reassembleSkillFile(skill) {
 ...
 */
export function computeCompositeScore(gateResults, durationMs, tokenEstimate) {
    const scorable = gateResults.filter((g) => g.status !== 'skip');
    const gateScore = scorable.length === 0
        ? 0
...
export const reorderSections = {
    ...
    apply(body, rng = Math.random) {
        ...
        if (i === j)
            j = indices[(indices.indexOf(j) + 1) % indices.length];
        if (i === j)
            return null;
 ...
 */
export const sharpenInstructions = {
...
export const toggleOptionalSection = {
    ...
    apply(body, rng = Math.random) {
        ...
        const optionalIndices = sections
            ...
            .filter(({ section, index }) => index > 0 && optionalKeywords.some((kw) => section.heading.includes(kw)))
            .map(({ index }) => index);
 ...
 */
export const consolidateConstraints = {
    ...
    apply(body) {
        ...
        const constraintIndices = sections
            .map((s, i) => ({ section: s, index: i }))
            .filter(({ section, index }) => index > 0 && constraintKeywords.some((kw) => section.heading.includes(kw)))
            .map(({ index }) => index);
        ...
        for (let i = 1; i < sections.length; i++) {
            ...
            if (lines.length >= 3 && lines.length <= 8) {
                ...
                updated[i] = {
                    ...
                    body: sections[i].body.replace(lines.map((l) => l).join('\n'), '\n' + prose + '\n'),
                };
                return {
 ...
 */
export function evaluateSkillPrompt(body) {
    const results = [];
...
export function runExperiment(config) {
    ...
    const improvementPct = baseline.compositeScore > 0
...
export function insertExperiment(db, record) {
    db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(record.id, record.skill_name, record.scenario_description, record.started_at, record.completed_at, record.total_iterations, record.best_iteration, record.baseline_score, record.best_score, record.improvement_pct, record.status, record.config);
}
export function insertIteration(db, record) {
    db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(record.experiment_id, record.iteration_number, record.mutation_strategy, record.mutation_detail, record.gate_verdict, record.gate_pass_count, record.gate_fail_count, record.duration_ms, record.token_estimate, record.composite_score, record.kept, record.prompt_hash);
}
export function getExperimentResults(db, experimentId) {
    const experiment = db.prepare('SELECT * FROM optimization_experiments WHERE id = ?').get(experimentId);
    if (!experiment)
        return null;
    const iterations = db.prepare('SELECT * FROM optimization_iterations WHERE experiment_id = ? ORDER BY iteration_number').all(experimentId);
    return { experiment, iterations };
}
export function getRecentExperiments(db, limit = 10) {
    return db.prepare('SELECT * FROM optimization_experiments ORDER BY started_at DESC LIMIT ?').all(limit);
}
/**
 * Persist a full experiment result to the database.
 */
export function persistExperiment(db, result, scenario) {
    insertExperiment(db, {
...
export function formatExperimentReport(result) {
    ...
    for (const iter of result.iterations) {
        ...
        lines.push(`  ${String(iter.iteration).padStart(2)}. ${stratName} ${iter.compositeScore.toFixed(3)} (${deltaStr.padStart(5)}) ${status}`);
    }
    lines.push('');
