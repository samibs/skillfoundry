 ...
 */
function resolveAuditPath(workDir) {
...
export function detectActor() {
    ...
    if (process.env.GITHUB_ACTOR)
        return `github:${process.env.GITHUB_ACTOR}`;
    if (process.env.GITLAB_USER_LOGIN)
        return `gitlab:${process.env.GITLAB_USER_LOGIN}`;
    if (process.env.BUILD_REQUESTEDFOR)
        return `azdo:${process.env.BUILD_REQUESTEDFOR}`;
    if (process.env.CIRCLE_USERNAME)
        return `circleci:${process.env.CIRCLE_USERNAME}`;
    // Local user
 ...
 */
export function appendAuditEntry(workDir, entry) {
    const log = getLogger();
 ...
 */
export function auditGateResult(workDir, result, storyFile) {
 ...
 */
export async function streamAuditEntries(workDir, callback) {
    const { filePath } = resolveAuditPath(workDir);
    if (!existsSync(filePath)) {
        return 0;
    }
 ...
 */
export function countAuditEntries(workDir) {
    const { filePath } = resolveAuditPath(workDir);
    if (!existsSync(filePath))
        return 0;
    const content = readFileSync(filePath, 'utf-8');
 ...
 */
export function getRecentAuditEntries(workDir, count = 50) {
 ...
 */
export function filterAuditEntries(workDir, filters) {
    const all = readAuditLog(workDir);
    return all.filter((entry) => {
        if (filters.gate && entry.gate !== filters.gate)
            return false;
        if (filters.verdict && entry.verdict !== filters.verdict)
            return false;
 ...
 */
export function rotateAuditLogIfNeeded(workDir, threshold = ROTATION_THRESHOLD) {
 ...
 */
export function getAuditSummary(workDir) {
