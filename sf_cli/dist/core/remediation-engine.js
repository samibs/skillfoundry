...
import { initDatabase, getFailurePatterns, getProjectDetail, upsertPlaybook, getPlaybooks, matchPlaybooks, insertRemediation, updateRemediationStatus, getRemediations, getRemediationStats, recordPlaybookOutcome, updateFailureRemediationStatus, } from './dashboard-db.js';
// ── Built-in playbooks ──────────────────────────────────────────
 ...
 */
export function seedPlaybooks(db) {
    let seeded = 0;
 ...
 */
export function scanForRemediations(db, options) {
    const log = getLogger();
    const result = { actions_created: 0, actions_skipped: 0, auto_applied: 0, errors: [] };
    ...
    for (const failure of failures) {
        try {
            ...
            const existing = db.prepare("SELECT id FROM remediations WHERE failure_signature = ? AND project_id = (SELECT id FROM projects WHERE name = ?) AND status NOT IN ('completed', 'failed')").get(failure.signature, failure.project_name);
            if (existing) {
            ...
            if (categoryMatch && playbooks.length === 0) {
                const catPlaybooks = db.prepare('SELECT * FROM playbooks WHERE category = ? ORDER BY success_count DESC').all(categoryMatch);
 ...
 */
export function applyRemediation(db, remediationId, action, result) {
    const existing = db.prepare('SELECT * FROM remediations WHERE id = ?').get(remediationId);
    if (!existing) {
 ...
 */
export function generateRemediationReport(db, options) {
...
export function formatRemediationReport(report) {
    ...
    if (report.pending_actions.length > 0) {
        ...
        for (const action of report.pending_actions) {
            ...
            if (action.steps) {
                const steps = JSON.parse(action.steps);
 ...
 */
export function formatRemediationList(actions) {
    if (actions.length === 0) {
        return [
            'Remediations',
            LINE.repeat(60),
 ...
 */
export function formatPlaybookList(playbooks) {
    ...
    for (const pb of playbooks) {
        ...
        if (steps.length > 3)
            lines.push(`      ... and ${steps.length - 3} more`);
        lines.push('');
    }
    return lines.join('\n');
}
 ...
 */
function matchByCategoryHeuristic(signature) {
    const lower = signature.toLowerCase();
    if (lower.startsWith('build:'))
        return 'BUILD_FAILURE';
    if (lower.startsWith('test:'))
        return 'TEST_GAP';
    if (lower.startsWith('security') || lower.startsWith('specter:'))
        return 'SECURITY';
    if (lower.startsWith('gate:'))
        return 'GATE_FAILURE';
    if (lower.startsWith('pipeline:'))
        return 'PIPELINE';
    if (lower.includes('circuit_breaker'))
        return 'CIRCUIT_BREAKER';
    if (lower.includes('dep') || lower.includes('dependency'))
        return 'DEPENDENCY';
    return null;
}
/**
 ...
 */
export function runRemediationScan(dbPath, autoApply = false) {
