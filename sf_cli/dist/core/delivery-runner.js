...
import { assignBudget, chooseTestScope, shouldRunValidation, saveValidationEvidence, checkCompletion, loadDeliverySettings, } from './delivery.js';
...
import { measureImpact, describeImpact } from './delivery-impact.js';
import { recordHandoff } from './delivery-context.js';
 ...
 */
export function detectChangedFiles(workDir, baseRef) {
    const files = new Set();
    if (baseRef) {
        const base = revParse(workDir, baseRef);
        const head = revParse(workDir, 'HEAD');
        if (base && head && base !== head) {
            for (const f of changedFilesBetween(workDir, base, head))
                files.add(f);
        }
    }
 ...
 */
export function runValidation(workDir, input) {
 ...
 */
export async function runOrReuse(workDir, spec, execute, passed) {
    const key = evidenceKey(spec.kind, spec.command, spec.scope);
    const decision = shouldRunValidation(workDir, { kind: spec.kind, command: spec.command, scope: spec.scope, changedFiles: spec.changedFiles }, spec.owner);
    if (decision.action === 'REUSE') {
        ...
        const recorded = decision.evidence?.payload;
    ...
    if (decision.action === 'FIX_FIRST') {
        ...
        return {
            ...
            value: decision.evidence?.payload,
 ...
 */
export function executeTask(workDir, plan, validations, opts = {}) {
    const owner = opts.owner ?? plan.taskId;
    const outcomes = [];
    const escalations = [];
    let budget = plan.budget;
    let scope = plan.scope;
    const applicable = validations.filter((v) => !v.minScope || SCOPE_RANK[scope] >= SCOPE_RANK[v.minScope]);
    for (const v of applicable) {
        ...
        if (outcome.action === 'BLOCKED_KNOWN_FAILURE')
            break;
    }
 ...
 */
export function completeTask(workDir, execution, opts = {}) {
...
export function taskIsComplete(workDir, execution, opts = {}) {
...
export function deliveryRunnerEnabled(workDir) {
