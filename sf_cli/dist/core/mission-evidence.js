 ...
 */
export function writeEvidence(workDir, missionId, kind, payload) {
    assertMissionId(missionId);
    if (!isEvidenceKind(kind)) {
        throw new Error(`Unknown evidence kind "${kind}". Allowed: ${EVIDENCE_KINDS.join(', ')}`);
    }
    const redacted = JSON.parse(redactText(JSON.stringify(payload), true));
    const serialized = JSON.stringify(redacted);
    if (Buffer.byteLength(serialized, 'utf-8') > MAX_PAYLOAD_BYTES) {
        throw new Error(`Evidence payload for ${missionId}/${kind} is ${Buffer.byteLength(serialized, 'utf-8')} bytes ` +
...
export function readEvidence(workDir, missionId, kind) {
 ...
 */
export function classifyProcessState(signals) {
    if (!signals.alive) {
        if (signals.exitCode === 0)
            return 'COMPLETED';
        return 'FAILED';
    }
    if (signals.elapsedMs >= signals.timeoutMs)
        return 'STALLED';
    const QUIET_THRESHOLD_MS = 60_000;
    if (signals.msSinceOutput < QUIET_THRESHOLD_MS)
        return 'RUNNING';
 ...
 */
export async function runValidatedCommand(opts) {
    ...
    await Promise.race([
        ...
        new Promise((r) => setTimeout(r, STDIO_DRAIN_MS)),
    ]);
    ...
    const orphans = pgid > 0 && !result.spawnError
        ? checkOrphans(pgid)
        : { check: 'UNSUPPORTED', detail: 'Child was never spawned' };
 ...
 */
export function isCleanValidation(evidence, opts = {}) {
    const reasons = [];
    if (opts.expectTests && evidence.passed === undefined && evidence.failed === undefined) {
        reasons.push('No test results were detected in the output — a zero exit code here proves nothing. ' +
    ...
    if (!evidence.normal_termination) {
        reasons.push(evidence.timed_out
 ...
 */
export function classifyFailure(output, exitCode) {
 ...
 */
export function adjudicatePreExisting(baseline, failureSignature) {
    if (!baseline) {
        return {
            substantiated: false,
            reason: 'No baseline run was recorded. A "pre-existing" claim requires the same test, in the same environment, ' +
    ...
    return {
        substantiated: false,
        reason: `The baseline run produced a different failure signature (${baselineSignature.slice(0, 16)} vs ${failureSignature.slice(0, 16)}). ` +
...
export class RetryTracker {
     ...
     */
    record(signature, stateChanged, justification) {
        this.attempts.push({
    ...
    shouldRetry() {
        ...
        if (previous && previous.failure_signature === last.failure_signature && !last.state_changed) {
            return {
                allowed: false,
                reason: 'NO_PROGRESS: two consecutive attempts produced identical evidence with no state change. ' +
...
//# sourceMappingURL=mission-evidence.js.map