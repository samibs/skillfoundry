 ...
 */
export function normaliseSemgrepFindings(report) {
 ...
 * @returns Array of normalised findings.
 */
export function normaliseGitleaksFindings(result) {
    return result.findings.map((f) => {
        const ruleLC = f.rule.toLowerCase();
        const severity = ruleLC.includes('private') || ruleLC.includes('rsa') || ruleLC.includes(' ec ')
            ? 'critical'
            : 'high';
        return {
            id: randomUUID(),
 ...
 * @returns Array of normalised findings.
 */
export function normaliseCheckovFindings(result) {
    return result.findings.map((f) => ({
        id: randomUUID(),
        scanner: 'checkov',
        severity: f.severity,
 ...
 * @returns Array of normalised findings.
 */
export function normaliseDependencyFindings(report) {
 ...
 * @returns Array of normalised findings.
 */
export function normaliseLicenseFindings(result) {
    return result.findings.map((f) => ({
        id: randomUUID(),
 ...
 */
function completedSection(name, findings, duration) {
    return {
        name,
        status: 'completed',
        duration,
 ...
 */
function skippedSection(name, reason) {
    return {
        name,
        status: 'skipped',
        statusReason: reason,
 ...
 */
function failedSection(name, reason) {
    return {
        name,
        status: 'failed',
        statusReason: reason,
    ...
    generate(results) {
        ...
        try {
            if (!results.gitleaks) {
                ...
                gitleaksSection = skippedSection('Gitleaks', results.gitleaks.skipReason ?? 'Gitleaks was skipped');
            }
            else if (!results.gitleaks.success) {
                gitleaksSection = failedSection('Gitleaks', results.gitleaks.skipReason ?? 'Gitleaks scan failed');
            }
            else {
        ...
        try {
            if (!results.checkov) {
                ...
                checkovSection = skippedSection('Checkov', results.checkov.skipReason ?? 'Checkov was skipped');
            }
            else if (!results.checkov.success) {
                checkovSection = failedSection('Checkov', results.checkov.skipReason ?? 'Checkov scan failed');
            }
            else {
 ...
 */
function renderScannerSection(section, sectionId) {
    ...
    const findingsTable = (findings, suppressedGroup) => {
 ...
 */
export function createUnifiedReportGenerator(projectPath) {
 ...
 */
export function getDefaultReportPaths(projectPath, timestamp) {
...
//# sourceMappingURL=unified-security-report.js.map