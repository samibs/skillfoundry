...
import { runExperiment, formatExperimentReport, formatStrategyList, parseSkillFile, reassembleSkillFile, persistExperiment, getRecentExperiments, getExperimentResults, } from '../core/skill-optimizer.js';
...
function findSkillFile(frameworkDir, skillName) {
    ...
    try {
        ...
        for (const f of files) {
            if (!f.endsWith('.md') || f.startsWith('_') || f.startsWith('INDEX'))
                continue;
...
export const optimizeCommand = {
    ...
    async execute(args, session) {
        ...
        switch (subcommand) {
            ...
            case 'history': {
                ...
                try {
                    ...
                    if (experiments.length === 0)
                        return '  No optimization experiments found.';
                    const lines = [
            ...
            case 'result': {
                if (!skillName)
                    return '  Usage: /optimize result <experiment-id>';
                const db = initDatabase(dbPath);
                try {
                    // Try prefix match
                    ...
                    if (!match)
                        return `  Experiment "${skillName}" not found.`;
