...
import { loadCredentials, setCredential, removeCredential, getCredentialsPath, injectCredentials, hasAnyCredentials, PROVIDER_KEY_URLS, } from '../core/credentials.js';
import { AVAILABLE_PROVIDERS, detectAvailableProviders, } from '../core/provider.js';
export function runSetupNonInteractive(opts) {
    if (opts.list) {
    ...
    if (!AVAILABLE_PROVIDERS[opts.provider]) {
        return (`Unknown provider: ${opts.provider}\n` +
...
function listConfiguredProviders() {
    ...
    for (const [key, info] of Object.entries(AVAILABLE_PROVIDERS)) {
        const hasCred = store[key] &&
            Object.values(store[key]).some((v) => v);
        ...
        if (!hasEnv && !hasCred && key !== 'ollama' && key !== 'lmstudio') {
            lines.push(`    Get key: ${PROVIDER_KEY_URLS[key] || 'N/A'}`);
        }
    }
    lines.push('');
...
export async function runInteractiveSetup() {
    ...
    try {
        ...
        const providers = Object.entries(AVAILABLE_PROVIDERS).filter(([k]) => !localProviders.includes(k));
    ...
    execute: async (args, _session) => {
        ...
        if (sub === 'remove') {
            ...
            if (!provider || !AVAILABLE_PROVIDERS[provider]) {
                return ('Usage: /setup remove <provider>\n' +
                    `Available: ${Object.keys(AVAILABLE_PROVIDERS).join(', ')}`);
            }
        ...
        injectCredentials();
        return (`Saved ${field} for ${provider}. Provider is now ready to use.\n` +
