    ...
    async execute(args, session) {
        ...
        switch (subcommand) {
            ...
            case 'info': {
                ...
                if (!packName)
                    return '  Usage: /domain info <pack-name>';
                const packs = listInstalledPacks(frameworkDir);
                const pack = packs.find((p) => p.metadata.name === packName);
                if (!pack)
                    return `  Pack "${packName}" not found. Run /domain list to see installed packs.`;
                const lines = [
            ...
            case 'matrix': {
                const query = positional.join(' ');
                if (!query)
                    return '  Usage: /domain matrix <matrix-name> [--pack name]';
                const packs = listInstalledPacks(frameworkDir);
                for (const pack of packs) {
                    if (flags.pack && pack.metadata.name !== flags.pack)
                        continue;
                    const matrix = loadMatrix(pack.path, query);
                    if (matrix)
                        return formatMatrixData(matrix);
                }
            ...
            case 'validate': {
                ...
                if (!filePath)
                    return '  Usage: /domain validate <file> --pack <name>';
            ...
            case 'prd': {
                ...
                if (!description)
                    return '  Usage: /domain prd <description>';
