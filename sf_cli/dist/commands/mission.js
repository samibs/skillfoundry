...
import { initLedger, upsertMission, getMission, updateMission, readLedger, setDimension, attachEvidence, setAcceptanceCriteria, setProvenance, openGap, closeGap, openGaps, readGaps, reconcileLedger, initAiTree, assertMissionId, isLedgerDimension, isMissionStatus, LEDGER_DIMENSIONS, MISSION_STATUSES, PATCHES_DIR, AC_DISPOSITIONS, setExecutionStatus, setExecutionPlan, recordBlocker, clearBlockers, setBaseline, dispatchWave, completeWave, recordIntegration, isExecutionStatus, isDependencyKind, isFailureCode, EXECUTION_STATUSES, FAILURE_CODES, DEPENDENCY_KINDS, } from '../core/mission-ledger.js';
import { registerAgent, releaseAgent, listAgents, activeAgents, readAgent, planWave, integrationOrder, evaluateEligibility, orchestrationSummary, sharedWorktreeViolations, } from '../core/mission-orchestration.js';
import { readCatalog, upsertApplication, detectPortConflicts, serviceForPort, registerProcess, listProcesses, listAllProcesses, stopOwnedProcesses, orphanedProcesses, pruneProcessRecords, verifyProcessOwnership, isProcessAlive, } from '../core/mission-processes.js';
import { attestWorker, gateOnAttestation, listAttestations, relativeAttestationPath, isAgentKind, } from '../core/mission-attestation.js';
import { classifyBaseline, classifyWorkerStaleness, verifyProvenance, verifyPublication, analyzeCollision, } from '../core/mission-provenance.js';
import { runValidatedCommand, writeEvidence, readEvidence, isCleanValidation, classifyFailure, isEvidenceKind, } from '../core/mission-evidence.js';
import { stablePatchId, changedFiles, changedFilesBetween, revParse, treeSha, currentBranch, isGitRepo, topLevel, commitsBetween, rangePatchId, } from '../core/mission-git.js';
/**
...
function bad(s) { return `${RED}✗${RESET} ${s}`; }
...
function statusColor(status) {
    if (['COMPLETE', 'PASS', 'INTEGRATION_VALIDATED', 'PUBLISHED'].includes(status))
        return `${GREEN}${status}${RESET}`;
    if (status.startsWith('BLOCKED') || status === 'FAIL' || status === 'IMPLEMENTATION_GAP')
        return `${RED}${status}${RESET}`;
    if (['EVIDENCE_PARTIAL', 'EXTERNAL_VALIDATION_REQUIRED', 'INTEGRATION_READY'].includes(status))
        return `${YELLOW}${status}${RESET}`;
    return `${DIM}${status}${RESET}`;
}
function requireMissionId(positional, sub) {
    const id = positional[1];
    if (!id)
        throw new Error(`Usage: /mission ${sub} <MISSION-ID> …`);
    assertMissionId(id);
    return id;
}
// ── Patch guide (§14) ─────────────────────────────────────────────────────────
/** Repo-relative path of a mission's patch guide. */
function patchGuidePath(workDir, missionId) {
 ...
 */
function scaffoldPatchGuide(workDir, missionId, title) {
 ...
 */
function evaluateDod(workDir, missionId, mission) {
    ...
    gates.push({
        ...
        passed: mission.acceptance_criteria.length > 0 &&
            mission.acceptance_criteria.every((ac) => AC_DISPOSITIONS.includes(ac.disposition)),
    ...
    const tests = readEvidence(workDir, missionId, 'tests');
    gates.push({
...
function finalDisposition(mission, dodBlockers) {
    if (mission.publication_status === 'PUBLISHED')
        return 'PUBLISHED — COMPLETE';
    if (mission.integration_status === 'INTEGRATION_VALIDATED')
        return 'INTEGRATION_VALIDATED — READY_FOR_PUBLICATION';
    if (mission.implementation_status === 'FAIL' || mission.acceptance_status === 'FAIL')
        return 'FAILED — DO_NOT_INTEGRATE';
    if (mission.implementation_status === 'IMPLEMENTATION_GAP')
        return 'IMPLEMENTATION_GAP — REMEDIATION_REQUIRED';
    if (mission.implementation_status.startsWith('BLOCKED'))
        return mission.implementation_status;
    if (mission.external_validation_status === 'EXTERNAL_VALIDATION_REQUIRED')
        return 'EXTERNAL_VALIDATION_REQUIRED';
    if (mission.acceptance_status === 'EVIDENCE_PARTIAL')
        return 'EVIDENCE_PARTIAL — VALIDATION_REQUIRED';
    if (mission.implementation_status === 'COMPLETE' && dodBlockers === 0)
        return 'COMPLETE — INTEGRATION_READY';
    if (mission.implementation_status === 'COMPLETE')
        return 'EVIDENCE_PARTIAL — VALIDATION_REQUIRED';
...
function handleAttest(workDir, args) {
    ...
    const agent = isAgentKind(agentRaw) ? agentRaw : 'unknown';
    ...
    if (getMission(workDir, missionId)) {
        ...
        updateMission(workDir, missionId, (m) => {
            if (!m.attestations.includes(rel))
                m.attestations.push(rel);
    ...
    lines.push('');
    for (const w of attestation.warnings)
        lines.push(`  ${warn(w)}`);
    if (attestation.status === 'PASS') {
        ...
        for (const b of attestation.blockers)
            lines.push(`    ${RED}·${RESET} ${b}`);
    }
    lines.push('');
    return lines.join('\n');
}
function handleBaseline(workDir, args) {
    ...
    if (report.expected_sha)
        lines.push(`  Expected:    ${report.expected_sha.slice(0, 12)}`);
    ...
    if (report.remote_changed_files.length > 0) {
        ...
        for (const f of report.remote_changed_files.slice(0, 15))
...
async function handleEvidence(workDir, args) {
    ...
    if (fromFile) {
        ...
        if (!existsSync(abs))
            throw new Error(`Payload file not found: ${fromFile}`);
        const payload = JSON.parse(readFileSync(abs, 'utf-8'));
...
function handleAc(workDir, args) {
    const missionId = requireMissionId(args.positional, 'ac');
    const file = flagString(args.flags, 'file');
    if (!file)
        throw new Error('Usage: /mission ac <MISSION-ID> --file <criteria.json>');
    const abs = resolve(workDir, file);
    if (!existsSync(abs))
        throw new Error(`Criteria file not found: ${file}`);
...
function handleCommit(workDir, args) {
    ...
    if (!sha)
        throw new Error(`Could not resolve "${shaArg}" to a commit.`);
    ...
    if (baseArg && !base)
        throw new Error(`Could not resolve base ref "${baseArg}" to a commit.`);
    ...
    updateMission(workDir, missionId, (m) => {
        m.worker_sha = sha;
        if (base)
            m.execution.base_sha = base;
        if (patchId)
            m.stable_patch_id = patchId;
    });
...
function handleProvenance(workDir, args) {
    ...
    if (!workerSha)
        throw new Error('No worker SHA. Run `/mission commit <ID>` first, or pass --worker <sha>.');
    ...
    const verification = verifyProvenance(workDir, workerSha, integration, {
        ...
        includeGovernanceState: args.flags['include-governance-state'] === true ||
    ...
    if (rec.excluded_artifacts?.length) {
        ...
        for (const f of rec.excluded_artifacts)
            lines.push(`    ${DIM}·${RESET} ${f}`);
    }
    lines.push(`  Evidence:         ${rel}`);
    lines.push('');
    if (staleness.colliding_files.length > 0) {
        ...
        for (const f of staleness.colliding_files.slice(0, 10))
            lines.push(`    ${YELLOW}·${RESET} ${f}`);
        lines.push('');
    }
    if (verification.proven) {
        ...
        for (const b of verification.blockers)
            lines.push(`    ${RED}·${RESET} ${b}`);
        lines.push('');
...
function handlePublishCheck(workDir, args) {
    ...
    if (!expectedSha)
        throw new Error('Could not determine the SHA to verify. Pass --sha <sha>.');
...
function handleSet(workDir, args) {
    ...
    const result = setDimension(workDir, missionId, dimension, status, { force });
...
function handleGap(workDir, args) {
    ...
    if (action === 'open') {
        ...
        const type = (flagString(args.flags, 'type') ?? 'IMPLEMENTATION');
    ...
    if (action === 'close') {
        ...
        if (!id)
            throw new Error('Usage: /mission gap close <GAP-ID>');
...
function handleCollide(args) {
    ...
    const report = analyzeCollision(nameA, filesA.split(',').map((s) => s.trim()).filter(Boolean), nameB, filesB.split(',').map((s) => s.trim()).filter(Boolean), { dependency: args.flags.dependency === true || args.flags.dependency === 'true' });
...
function handleVerify(workDir, args) {
...
function handleReport(workDir, args) {
    ...
    if (mission.evidence.length === 0 && mission.attestations.length === 0) {
        ...
        for (const a of mission.attestations)
            md.push(`- \`${a}\``);
        for (const e of mission.evidence)
            md.push(`- \`${e}\``);
        if (mission.patch_guide)
            md.push(`- \`${mission.patch_guide}\``);
    }
    md.push('');
...
function handleReconcile(workDir) {
...
function handleAgent(workDir, args) {
    const action = args.positional[1] ?? 'list';
    if (action === 'register') {
    ...
    if (action === 'release') {
        const name = args.positional[2] ?? flagString(args.flags, 'name');
        if (!name)
            throw new Error('Usage: /mission agent release <name> [--status COMPLETED|FAILED]');
        ...
        if (orphans.length > 0 && args.flags.force !== true) {
            ...
            for (const o of orphans)
                lines.push(`    ${RED}·${RESET} pid ${o.pid}  ${o.command} ${o.args.join(' ')}`.slice(0, 110));
            lines.push('');
        ...
        const released = releaseAgent(workDir, name, statusRaw);
        return released
    ...
    if (action === 'show') {
        ...
        if (!name)
            throw new Error('Usage: /mission agent show <name>');
    ...
    if (agents.length === 0) {
        ...
        for (const a of agents) {
            lines.push(`  ${a.name.padEnd(30)}${a.mode.padEnd(11)}${a.workItem.padEnd(14)}${a.status.padEnd(11)}${a.worktree ?? '—'}`);
        }
    }
...
function handlePlan(workDir, args) {
    ...
    if (!missionId)
        throw new Error('Usage: /mission plan <WORK-ITEM> [--depends "AF-301:HARD,AF-302:SOFT"] [--writes "src/a.ts,src/b.ts"] [--prd P] [--story S]');
    assertMissionId(missionId);
    const dependencies = flagList(args.flags, 'depends').map((token) => {
    ...
    const plan = {};
    if (args.flags.depends !== undefined)
        plan.dependencies = dependencies;
    if (args.flags.writes !== undefined)
        plan.write_manifest = flagList(args.flags, 'writes');
    if (args.flags.prd !== undefined)
        plan.prd = flagString(args.flags, 'prd');
    if (args.flags.story !== undefined)
        plan.story = flagString(args.flags, 'story');
    if (args.flags.wave !== undefined)
        plan.wave = flagString(args.flags, 'wave');
    ...
    for (const f of execution.write_manifest.slice(0, 15))
        lines.push(`    ${DIM}·${RESET} ${f}`);
    lines.push('');
    ...
    for (const r of verdict.reasons)
        lines.push(`    ${YELLOW}·${RESET} ${r}`);
    lines.push('');
    return lines.join('\n');
}
function handleWave(workDir, args) {
    ...
    if (action === 'plan') {
        ...
        if (plan.cycles.length > 0) {
            ...
            for (const cycle of plan.cycles)
                lines.push(`    ${RED}·${RESET} ${cycle.join(' → ')}`);
            lines.push('');
            return lines.join('\n');
        }
    ...
    if (action === 'complete') {
        ...
        if (!waveId)
            throw new Error('Usage: /mission wave complete <WAVE-ID>');
    ...
    if (action === 'order') {
        const ledger = readLedger(workDir);
        if (!ledger)
            throw new Error('No ledger. Run /mission init first.');
    ...
    if (summary.dangling_dependencies.length > 0) {
        ...
        for (const d of summary.dangling_dependencies)
            lines.push(`    ${YELLOW}·${RESET} ${d.from} → ${d.to}`);
    }
    if (summary.cycles.length > 0) {
        lines.push('');
        lines.push(`  ${bad('Dependency cycle(s):')}`);
        for (const c of summary.cycles)
            lines.push(`    ${RED}·${RESET} ${c.join(' → ')}`);
    }
    lines.push('');
    return lines.join('\n');
}
function handleExec(workDir, args) {
    ...
    const force = args.flags.force === true || args.flags.force === 'true';
    const result = setExecutionStatus(workDir, missionId, status, { force });
...
function handleBlock(workDir, args) {
...
function handleCatalog(workDir, args) {
    ...
    if (action === 'add') {
        ...
        const services = specs.map((spec) => {
            ...
            if (!svcName)
                throw new Error(`Malformed --service "${spec}". Expected name:path:port[,port]`);
            return {
                name: svcName,
                path: svcPath,
        ...
        if (conflicts.length > 0) {
            ...
            for (const c of conflicts)
                lines.push(`    ${RED}·${RESET} port ${c.port} claimed by ${c.claimants.join(', ')}`);
        }
        lines.push('');
        return lines.join('\n');
    }
    if (action === 'port') {
        ...
        if (!Number.isInteger(port))
            throw new Error('Usage: /mission catalog port <number>');
...
async function handleProc(workDir, args) {
    const action = args.positional[1] ?? 'list';
    if (action === 'register') {
    ...
    if (action === 'stop') {
        ...
        if (!agent)
            throw new Error('Usage: /mission proc stop <agent> [--force]');
        const outcomes = await stopOwnedProcesses(workDir, agent, {
            force: args.flags.force === true || args.flags.force === 'true',
        });
        const lines = [head(`Stopping processes owned by ${agent}`)];
        if (outcomes.length === 0)
            lines.push(`  ${DIM}No processes recorded for this agent.${RESET}`);
    ...
    if (action === 'prune') {
        const agent = args.positional[2] ?? flagString(args.flags, 'agent');
        if (!agent)
            throw new Error('Usage: /mission proc prune <agent>');
    ...
    if (records.length === 0) {
        ...
        for (const r of records) {
            ...
            lines.push(`  ${String(r.pid).padEnd(9)}${r.status.padEnd(10)}${(alive ? 'yes' : 'no').padEnd(7)}${r.agent.padEnd(28)}${r.command} ${r.args.join(' ')}`.slice(0, 130));
...
function handleIntegrate(workDir, args) {
    ...
    if (!integrationSha)
        throw new Error(`Provenance for ${missionId} has no integration SHA.`);
    ...
    if (transition.accepted) {
        ...
        for (const b of transition.blockers)
            lines.push(`    ${RED}·${RESET} ${b}`);
    }
    lines.push('');
    return lines.join('\n');
}
function handleBaselineSet(workDir, args) {
    const ledger = readLedger(workDir);
    if (!ledger)
        throw new Error('No ledger. Run /mission init first.');
    ...
    if (!sha)
        throw new Error(`Could not resolve a SHA for "${branch}".`);
