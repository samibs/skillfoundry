...
import { analyzeTokens, compressContext, formatAnalysisReport, formatCompressReport, getAllCompressionStrategies, } from '../core/token-optimizer.js';
const LINE = '\u2501';
export const tokensCommand = {
    ...
    async execute(args, session) {
        ...
        switch (subcommand) {
            case 'analyze': {
                if (!target)
                    return '  Usage: /tokens analyze <file>';
            ...
            case 'compress': {
                if (!target)
                    return '  Usage: /tokens compress <file> [--strategies s1,s2] [--max-tokens N]';
                const filePath = resolve(session?.workDir || process.cwd(), target);
                if (!existsSync(filePath))
                    return `  File not found: ${filePath}`;
                const content = readFileSync(filePath, 'utf-8');
                const strategies = flags.strategies
                    ? flags.strategies.split(',')
                    : undefined;
