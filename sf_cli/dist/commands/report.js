...
import { UnifiedSecurityReportGenerator, getDefaultReportPaths, } from '../core/unified-security-report.js';
 ...
 */
function loadLatestBaseline(workDir) {
    ...
    const baselineEvents = events
        .filter((e) => e.event_type === 'benchmark_run' && e.details.baseline_snapshot === true)
    ...
    return {
        ...
        language_breakdown: d.language_breakdown ?? {},
    };
}
// ---------------------------------------------------------------------------
 ...
 */
async function handleSecurityReport(args, session) {
    let format = 'both';
    let outputDir = null;
    ...
    const lines = ['Running security scan across all scanners...', ''];
