...
import { generateJwt, decodeJwt, generateKeyPair, generateApiKey, generateUuid, generatePassword, generateSecret, generateHash, generateHmac, generateWebhookSecret, generateTotpSecret, generateEnvFile, getEnvTemplates, autoGenerateSecrets, formatJwtResult, formatKeyPairResult, formatAutoGenerateResult, } from '../core/generator-engine.js';
const LINE = '\u2501';
    ...
    async execute(args, session) {
        ...
        switch (subcommand) {
            case 'jwt': {
                ...
                if (flags.claims) {
                    try {
                        Object.assign(claims, JSON.parse(flags.claims));
                    }
                    catch { /* ignore */ }
                }
                const result = generateJwt({
                    algorithm: (flags.alg || flags.algorithm || 'HS256'),
                    secret: flags.secret,
            ...
            case 'keypair': {
                const alg = (flags.alg || flags.algorithm || 'RS256');
            ...
            case 'secret': {
                ...
                const encoding = (flags.encoding || 'hex');
                return `  ${generateSecret(length, encoding)}\n`;
            }
            case 'hash': {
                const input = positional[0] || flags.input || '';
                if (!input)
                    return '  Usage: /generate hash <input> [--algo sha256|sha512|scrypt]';
                const algo = (flags.algo || flags.algorithm || 'sha256');
            ...
            case 'hmac': {
                ...
                if (!data || !secret)
                    return '  Usage: /generate hmac --data <data> --secret <secret>';
                const algo = (flags.algo || 'sha256');
