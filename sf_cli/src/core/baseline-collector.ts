...
export interface BaselineSnapshot {
  ...
  lint_error_count: number;       // -1 = eslint not available
  type_error_count: number;       // -1 = tsc not available
  loc: number;                    // lines of code (non-empty lines in source files)
  file_count: number;             // total source files
  primary_language: string;       // detected from file extensions
  language_breakdown: Record<string, number>; // extension -> count
}
 ...
 */
export async function collectBaseline(workDir: string): Promise<BaselineSnapshot> {
  const log = getLogger();
 ...
 */
function detectPrimaryLanguage(breakdown: Record<string, number>): string {
  let maxCount = 0;
  let primary = 'Unknown';
 ...
 */
function countLinesOfCode(files: string[]): number {
  let total = 0;
  for (const file of files) {
    try {
 ...
 */
function countLintErrors(workDir: string, log: ReturnType<typeof getLogger>): number {
  ...
  let eslintBin: string | null = null;
 ...
 */
function countTypeErrors(workDir: string, log: ReturnType<typeof getLogger>): number {
 ...
 */
export function formatBaseline(snapshot: BaselineSnapshot): string {
