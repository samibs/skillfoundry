...
import type { TelemetryEvent, ForgeRunDetails } from './telemetry.js';
...
function computeStats(events: TelemetryEvent[]): ReportStats {
  ...
  for (const run of forgeRuns) {
    ...
    else if (run.status === 'warn') warnCount++;
    else if (run.status === 'fail') failCount++;
    else if (run.status === 'error') errorCount++;
...
function computeDailyTrends(events: TelemetryEvent[]): DailyTrend[] {
  const forgeRuns = events.filter((e) => e.event_type === 'forge_run');
  const byDate: Record<string, { passes: number; failures: number }> = {};

  for (const run of forgeRuns) {
    const date = run.timestamp.split('T')[0];
    if (!byDate[date]) byDate[date] = { passes: 0, failures: 0 };
...
function computeGateBreakdown(events: TelemetryEvent[]): GateBreakdown[] {
  const gateEvents = events.filter((e) => e.event_type === 'gate_execution');
  const byTier: Record<string, { total: number; passes: number; failures: number }> = {};

  for (const evt of gateEvents) {
    const tier = (evt.details as { tier?: string }).tier || 'unknown';
    if (!byTier[tier]) byTier[tier] = { total: 0, passes: 0, failures: 0 };
    byTier[tier].total++;
 ...
 */
export function generateHtmlReport(events: TelemetryEvent[], baseline?: BaselineSnapshot): string {
