...
export interface ForgeRunDetails {
  ...
  dependency_findings: { critical: number; high: number; moderate: number; low: number };
...
export interface SecurityScanDetails {
  ...
  top_findings: Array<{ file: string; line: number; rule: string; severity: string }>;
}
...
export interface DependencyScanDetails {
  ...
  findings: Array<{ name: string; version: string; severity: string; cve: string; advisory_url: string }>;
}
...
export interface BenchmarkRunDetails {
  tasks_count: number;
  governed_results: { security_findings: number; type_errors: number; banned_patterns: number; test_pass_rate: number };
  ungoverned_results: { security_findings: number; type_errors: number; banned_patterns: number; test_pass_rate: number };
  improvement_pct: { security: number; type_safety: number; patterns: number };
}
...
export interface TelemetryAggregation {
  ...
  total_security_findings: { critical: number; high: number; medium: number; low: number };
  total_dependency_findings: { critical: number; high: number; moderate: number; low: number };
...
function getTelemetryPath(workDir: string): string {
  return join(workDir, SF_DIR, TELEMETRY_FILE);
}

function ensureDir(workDir: string): void {
 ...
 */
function rotateIfNeeded(workDir: string): void {
 ...
 */
export function readEventsByType(workDir: string, type: TelemetryEventType): TelemetryEvent[] {
 ...
 */
export function aggregateMetrics(workDir: string, window: number = 10): TelemetryAggregation {
  const { events } = readEvents(workDir);
  ...
  for (const run of forgeRuns) {
    const d = run.details as Partial<ForgeRunDetails>;
    if (run.status === 'pass') successful++;
    else if (run.status === 'warn') partial++;
    else failed++;
 ...
 */
function calculateTrend(runs: TelemetryEvent[]): 'improving' | 'declining' | 'stable' {
  ...
  function avgPassRate(evts: TelemetryEvent[]): number {
    let passes = 0;
    let total = 0;
    for (const e of evts) {
      const d = e.details as Partial<ForgeRunDetails>;
 ...
 */
export function formatMetrics(agg: TelemetryAggregation): string {
  if (agg.total_runs === 0) {
    return [
      'Quality Metrics',
      '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
 ...
 */
export function formatMetricsWithBaselines(agg: TelemetryAggregation): string {
  const base = formatMetrics(agg);
  if (agg.total_runs === 0) return base;
