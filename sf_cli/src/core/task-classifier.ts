 ...
 */
function detectTaskType(lower: string): string | undefined {
...
export class JurisdictionError extends Error {
  constructor(
    ...
    public readonly taskType: string | undefined,
    public readonly complexity: TaskComplexity,
  ) {
    super(
      `Jurisdiction "${jurisdiction}" blocks cloud routing for this task. ` +
 ...
 */
export function selectProvider(prompt: string, config: RoutingConfig): RoutingDecision {
 ...
 */
export function checkOutputQuality(prompt: string, response: string): QualityCheckResult {
  // Check 1: non-empty
