...
export function ensureSmartRouterSchema(db: Database.Database): void {
  db.exec(SMART_ROUTER_SCHEMA);
}

export function recordDecision(db: Database.Database, decision: RoutingDecisionRecord): void {
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    decision.id, decision.task_description, decision.task_keywords,
    decision.agent_selected, decision.outcome, decision.score,
    decision.duration_ms, decision.cost_usd, decision.timestamp, decision.project_id,
  );
}
...
export function updateAgentPerformance(
  ...
  const existing = db.prepare(
    'SELECT * FROM agent_performance WHERE agent_name = ? AND task_type = ?',
  ).get(agentName, taskType) as AgentPerformanceRecord | undefined;
...
export function getAgentPerformance(db: Database.Database, agentName?: string): AgentPerformanceRecord[] {
  if (agentName) {
    return db.prepare('SELECT * FROM agent_performance WHERE agent_name = ? ORDER BY avg_score DESC')
      .all(agentName) as AgentPerformanceRecord[];
  }
  return db.prepare('SELECT * FROM agent_performance ORDER BY avg_score DESC')
    .all() as AgentPerformanceRecord[];
}

export function getRecentDecisions(db: Database.Database, limit = 20): RoutingDecisionRecord[] {
...
export function routeTask(
  ...
  const perfRecords = db.prepare(
    'SELECT * FROM agent_performance WHERE task_type = ? ORDER BY avg_score DESC',
  ).all(taskType) as AgentPerformanceRecord[];

  const validPerf = perfRecords.filter(
    (r) => availableAgents.includes(r.agent_name) && r.success_count + r.failure_count >= 3,
  );
  ...
  const pastDecisions = db.prepare(
    'SELECT * FROM routing_decisions WHERE outcome = ? ORDER BY score DESC LIMIT 50',
  ).all('success') as RoutingDecisionRecord[];

  let bestMatch: { agent: string; overlap: number; score: number } | null = null;

  for (const d of pastDecisions) {
    if (!availableAgents.includes(d.agent_selected)) continue;
 ...
 */
export function startAgentDispatch(
  db: Database.Database,
  description: string,
  agentName: string,
  projectId?: string,
 ...
 */
export function completeAgentDispatch(
  db: Database.Database,
  decisionId: string,
  ...
  const decision = db.prepare('SELECT * FROM routing_decisions WHERE id = ?').get(decisionId) as RoutingDecisionRecord | undefined;
  if (decision) {
 ...
 */
export function hasLearningData(db: Database.Database): boolean {
  ensureSmartRouterSchema(db);
  const row = db.prepare('SELECT COUNT(*) as cnt FROM routing_decisions WHERE outcome IS NOT NULL').get() as { cnt: number };
  return row.cnt >= 10;
}
 ...
 */
export function getLearningStatus(db: Database.Database): { totalDecisions: number; completedDecisions: number; uniqueAgents: number; isLearning: boolean } {
  ensureSmartRouterSchema(db);
  const total = (db.prepare('SELECT COUNT(*) as cnt FROM routing_decisions').get() as { cnt: number }).cnt;
  const completed = (db.prepare('SELECT COUNT(*) as cnt FROM routing_decisions WHERE outcome IS NOT NULL').get() as { cnt: number }).cnt;
  const agents = (db.prepare('SELECT COUNT(DISTINCT agent_name) as cnt FROM agent_performance').get() as { cnt: number }).cnt;
  return { totalDecisions: total, completedDecisions: completed, uniqueAgents: agents, isLearning: completed >= 10 };
}

// ── Formatting ──────────────────────────────────────────────────

export function formatRoutingReport(rec: RoutingRecommendation): string {
  const lines = [
    'Smart Router',
    LINE.repeat(60),
...
export function formatPerformanceTable(records: AgentPerformanceRecord[]): string {
  if (records.length === 0) return '  No agent performance data yet.\n';
...
export function formatDecisionHistory(records: RoutingDecisionRecord[]): string {
  if (records.length === 0) return '  No routing decisions recorded yet.\n';
