...
export class VectorStore {
  ...
  async add(documents: MemoryDocument[]): Promise<void> {
    ...
    for (let i = 0; i < documents.length; i++) {
      ...
      const vectorFile: VectorFile = { id: doc.id, vector: embedding.vector };
      writeFileSync(
        this.vectorFilePath(doc.id),
        JSON.stringify(vectorFile),
        'utf-8',
      );
      ...
      if (existingIds.has(doc.id)) {
        ...
        if (idx !== -1) index.entries[idx] = entry;
      } else {
  ...
  async search(
    ...
    const results: SearchResult[] = topResults.map(({ entry, score }) => ({
      id: entry.id,
      text: entry.text,
  ...
  private ensureInitialized(): void {
    if (!this.initialized) {
      throw new Error(
        'VectorStore is not initialized. Call initialize() before using the store.',
      );
    }
  }
  ...
  private readStats(): StatsFile | null {
  ...
  private writeStats(stats: StatsFile): void {
    const statsPath = join(this.storePath, STATS_FILE);
    writeFileSync(statsPath, JSON.stringify(stats, null, 2), 'utf-8');
  }

  private readVectorFile(id: string): VectorFile | null {
   ...
   */
  private loadSourceDocuments(): MemoryDocument[] {
    const log = getLogger();
  ...
  private entryToMemoryDocument(
    ...
    if (!id || !text || text.trim().length === 0) return null;
 ...
 */
function applyMetadataFilter(entries: IndexEntry[], filter: MetadataFilter): IndexEntry[] {
...
type DocType = MemoryDocument['metadata']['type'];
type DocScope = MemoryDocument['metadata']['scope'];

/**
 * Map a raw JSONL entry type string to a valid MemoryDocument type.
 * Unknown types default to 'fact'.
 */
function mapEntryType(raw: string): DocType {
  const valid: DocType[] = ['decision', 'error', 'pattern', 'fact', 'lesson'];
 ...
 */
function mapScope(raw: string): DocScope {
  const valid: DocScope[] = ['project', 'framework', 'global'];
  const lower = raw.toLowerCase() as DocScope;
