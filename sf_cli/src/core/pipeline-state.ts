...
import { GateBarrier, fromGateSummary, type GateErrorLog } from './state-gate-barrier.js';
...
export function storySliceName(storyFile: string): string {
 ...
 */
export function initRunState(runsDir: string, runId: string): StateKernel | null {
  try {
 ...
 */
export function streamStorySlice(
  kernel: StateKernel | null,
  storyFile: string,
  ...
  try {
    if (outcome.status === 'failed') {
      const errors: GateErrorLog[] = [
        { tier: 'FORGE', name: slice, detail: outcome.reason ?? 'story failed' },
      ];
      kernel.commitSlice(
        slice,
        'forge',
        { status: 'FAILED', error_logs: errors, cost_usd: outcome.costUsd, turns: outcome.turnCount },
        0,
      );
