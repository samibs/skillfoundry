...
import type { TelemetryEvent, TelemetryAggregation, ForgeRunDetails, SecurityScanDetails, DependencyScanDetails } from './telemetry.js';
 ...
 */
export function generateReport(workDir: string, window: number = 10): QualityReport {
  ...
  for (const scan of secScans) {
    const d = scan.details as Partial<SecurityScanDetails>;
    if (d.findings_by_owasp) {
      for (const [cat, count] of Object.entries(d.findings_by_owasp)) {
        owaspCoverage[cat] = (owaspCoverage[cat] || 0) + (count as number);
      }
    }
  }
  ...
  for (const scan of depScans) {
    const d = scan.details as Partial<DependencyScanDetails>;
    depVulnTotal += d.vulnerable_count ?? 0;
    for (const finding of d.findings || []) {
      const f = finding as { severity: string };
      if (f.severity === 'critical') depSummary.critical++;
      else if (f.severity === 'high') depSummary.high++;
      else if (f.severity === 'moderate') depSummary.moderate++;
      else depSummary.low++;
    }
  }
  ...
  let verdict = 'HEALTHY';
  if (successRate < 0.5) verdict = 'AT RISK';
  else if (successRate < 0.8) verdict = 'NEEDS ATTENTION';
  else if (agg.total_security_findings.critical > 0) verdict = 'SECURITY CONCERN';
 ...
 */
export function formatReportJson(report: QualityReport): string {
  return JSON.stringify(report, null, 2);
}

// ── Helper Functions ────────────────────────────────────────────

function computeGatePerformance(gateEvents: TelemetryEvent[]): QualityReport['gates'] {
  const byTier: Record<string, { passes: number; total: number; totalMs: number; failures: Record<string, number> }> = {};

  for (const evt of gateEvents) {
    const d = evt.details as { tier?: string; gate_name?: string; findings_count?: number };
    const tier = d.tier || 'unknown';
    if (!byTier[tier]) byTier[tier] = { passes: 0, total: 0, totalMs: 0, failures: {} };

    byTier[tier].total++;
    byTier[tier].totalMs += evt.duration_ms;
    if (evt.status === 'pass') byTier[tier].passes++;
    if (evt.status === 'fail') {
...
function computeTrends(
  recentRuns: TelemetryEvent[],
  olderRuns: TelemetryEvent[],
): { direction: string; passRateDelta: number; securityDelta: number } {
  function avgPassRate(runs: TelemetryEvent[]): number {
    let passes = 0, total = 0;
    for (const r of runs) {
      const d = r.details as Partial<ForgeRunDetails>;
  ...
  function totalSecFindings(runs: TelemetryEvent[]): number {
    let total = 0;
    for (const r of runs) {
      const d = r.details as Partial<ForgeRunDetails>;
      const sf = d.security_findings;
      if (sf) total += (sf.critical ?? 0) + (sf.high ?? 0) + (sf.medium ?? 0) + (sf.low ?? 0);
    }
    return total;
  }
  ...
  if (passRateDelta > 0.05) direction = 'improving';
  else if (passRateDelta < -0.05) direction = 'declining';

  return { direction, passRateDelta, securityDelta };
}

function computeBaselines(agg: TelemetryAggregation): QualityReport['baselines'] {
  if (agg.total_runs === 0) return [];
...
function generateRecommendations(agg: TelemetryAggregation, baselines: QualityReport['baselines']): string[] {
  const recs: string[] = [];
