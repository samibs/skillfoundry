...
// ---------------------------------------------------------------------------
// Raw Gitleaks JSON schema (subset of actual output)
// ---------------------------------------------------------------------------
...
interface GitleaksRawFinding {
  ...
  Entropy?: number;
  Fingerprint?: string;
  // Gitleaks v8+ uses lower-camel too — accept both
  description?: string;
  file?: string;
  startLine?: number;
  endLine?: number;
 ...
 */
export function parseGitleaksVersion(versionOutput: string): [number, number, number] {
 ...
 */
export function isSupportedVersion(version: [number, number, number]): boolean {
  const [major, minor] = version;
  if (major > MIN_VERSION_MAJOR) return true;
  if (major === MIN_VERSION_MAJOR && minor >= MIN_VERSION_MINOR) return true;
  return false;
}
 ...
 */
export function findGitleaksBinary(): string | null {
...
export function parseGitleaksOutput(
  ...
  return raw.map((r): GitleaksFinding => {
 ...
 */
export function loadGitleaksIgnore(targetPath: string): Set<string> {
 ...
 */
export function createGitleaksScanner(projectRoot: string): GitleaksScanner {
 ...
 */
export function getGitleaksInstallInstructions(): string {
 ...
 */
export function _writeTestReport(path: string, findings: GitleaksRawFinding[]): void {
  const dir = join(path, '..');
