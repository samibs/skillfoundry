...
function messageTokens(msg: AnthropicMessage, charsPerToken = DEFAULT_CHARS_PER_TOKEN): number {
 ...
 */
export function compressSystemPrompt(prompt: string, maxTokens: number): string {
...
export function compactMessages(
  ...
  const {
    contextWindow,
    systemPromptBudget = Math.floor(contextWindow * 0.3), // 30% for system prompt
