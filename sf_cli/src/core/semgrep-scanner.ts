...
// Semgrep JSON output types (subset of the actual schema)
 ...
 */
export function mapSeverity(semgrepSeverity: string, confidence?: string): FindingSeverity {
 ...
 */
export function extractOwaspCategory(match: SemgrepMatch): string {
  ...
  if (meta?.owasp && meta.owasp.length > 0) {
    ...
    const canonical = OWASP_CATEGORIES.find((c) =>
      c.toLowerCase().includes(owaspTag.toLowerCase()) ||
 ...
 */
export function runSemgrepScan(options: ScanOptions): SemgrepResult | null {
  ...
  // Build command
  const configs: string[] = ['p/owasp-top-ten'];
  ...
  if (includeCustomRules && existsSync(customRulesDir)) {
    try {
      const ruleFiles = readdirSync(customRulesDir).filter(
 ...
 */
export function parseSemgrepOutput(jsonOutput: string): SemgrepResult {
 ...
 */
export function convertMatch(match: SemgrepMatch, workDir: string): SecurityFinding {
 ...
 */
export function runRegexScan(target: string): SecurityFinding[] {
...
export function generateSecurityReport(
  ...
  for (const f of findings) {
    const owaspMatch = OWASP_CATEGORIES.find((c) =>
      f.category.includes(c.split(' ')[0]),
    );
    if (owaspMatch) coveredCategories.add(owaspMatch);
  }
 ...
 */
export function runSecurityScan(target: string, timeout?: number): SecurityReport {
  ...
  if (semgrepResult && semgrepResult.results) {
    ...
    return generateSecurityReport(
      ...
      target,
      scanDurationMs,
      semgrepResult.version || detectSemgrep() || 'semgrep',
    );
  }
 ...
 */
export function getSemgrepInstallInstructions(): string {
