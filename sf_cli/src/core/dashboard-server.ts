...
import type { KpiSnapshot } from './kpi-engine.js';
import {
  scanForRemediations,
  applyRemediation,
...
function loadDashboardHtml(htmlPath?: string): string {
...
function routeApi(
  ...
  if (pathname === '/api/top' && method === 'GET') {
    const metric = (query.by || 'events') as 'events' | 'failures' | 'cost' | 'perf';
  ...
  if (pathname === '/api/trends/forecast' && method === 'GET') {
    ...
    const metric = (query.metric || 'success_rate') as keyof KpiSnapshot;
  ...
  if (remediationMatch && method === 'POST') {
    const action = (query.action || 'start') as 'start' | 'complete' | 'fail' | 'skip';
 ...
 */
export function startServer(options: ServerOptions): { stop: () => void; port: number } {
  const log = getLogger();
