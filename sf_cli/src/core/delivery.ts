...
import {
  classifyDeliveryBudget, escalateBudget, suggestEscalation,
  type ClassificationInput, type DeliveryBudget, type DeliveryBudgetLevel,
} from './delivery-budget.js';
import {
  selectTestScope, evaluateCompletion, policyFor,
  type ScopeInput, type ScopeDecision, type CompletionInput, type DeliveryComplete,
  type ExecutionPolicy,
} from './delivery-policy.js';
import {
  decideValidation, recordEvidence, lookupEvidence, claimValidation, evidenceKey,
  type EvidenceQuery, type ValidationDecision, type RecordEvidenceInput, type EvidenceEntry,
} from './delivery-evidence.js';
import { getLogger } from '../utils/logger.js';

/** Resolve the effective settings, filling defaults for a config written before this layer. */
export function deliverySettings(config: SfConfig): DeliveryEfficiencyConfig {
  return { ...DEFAULT_DELIVERY_EFFICIENCY, ...(config.delivery_efficiency ?? {}) };
}

/** Load the effective settings straight from a workspace. */
export function loadDeliverySettings(workDir: string): DeliveryEfficiencyConfig {
  return deliverySettings(loadConfig(workDir));
}

/** True when the layer is active for this workspace. */
export function isDeliveryEfficiencyEnabled(workDir: string): boolean {
 ...
 */
export function assignBudget(workDir: string, input: ClassificationInput): DeliveryBudget {
...
export function executionPolicy(budget: DeliveryBudgetLevel): ExecutionPolicy {
 ...
 */
export function chooseTestScope(workDir: string, input: ScopeInput): ScopeDecision {
  const settings = loadDeliverySettings(workDir);
 ...
 */
export function saveValidationEvidence(workDir: string, input: RecordEvidenceInput): EvidenceEntry | null {
  if (!loadDeliverySettings(workDir).enabled) return null;
 ...
 */
export function checkCompletion(workDir: string, input: CompletionInput): DeliveryComplete {
  const settings = loadDeliverySettings(workDir);
