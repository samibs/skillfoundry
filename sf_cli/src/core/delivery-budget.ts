...
export const DELIVERY_BUDGETS = ['LOW', 'MEDIUM', 'HIGH'] as const;

export type DeliveryBudgetLevel = (typeof DELIVERY_BUDGETS)[number];

/** Runtime guard for the budget vocabulary. */
export function isDeliveryBudgetLevel(value: unknown): value is DeliveryBudgetLevel {
  return typeof value === 'string' && (DELIVERY_BUDGETS as readonly string[]).includes(value);
}

/** Ordering, so escalation and comparison are unambiguous. */
const RANK: Record<DeliveryBudgetLevel, number> = { LOW: 0, MEDIUM: 1, HIGH: 2 };

/** True when `a` demands at least as much validation as `b`. */
export function atLeast(a: DeliveryBudgetLevel, b: DeliveryBudgetLevel): boolean {
  return RANK[a] >= RANK[b];
}

/** The higher of two budgets. */
export function maxBudget(a: DeliveryBudgetLevel, b: DeliveryBudgetLevel): DeliveryBudgetLevel {
...
export type BudgetSource =
  | 'EXPLICIT_OVERRIDE'
  | 'SAFETY_CRITICAL_PATH'
  | 'SAFETY_CRITICAL_KEYWORD'
  | 'LOW_RISK_PATH'
  | 'LOW_RISK_KEYWORD'
  | 'CHANGE_BREADTH'
  | 'DEFAULT'
 ...
 */
const SAFETY_CRITICAL_PATHS: Array<{ re: RegExp; label: string }> = [
 ...
 */
const SAFETY_CRITICAL_KEYWORDS: Array<{ re: RegExp; label: string }> = [
...
const LOW_RISK_PATHS: Array<{ re: RegExp; label: string }> = [
...
const LOW_RISK_KEYWORDS: Array<{ re: RegExp; label: string }> = [
...
function safetyCriticalSignals(input: ClassificationInput): string[] {
  const signals = new Set<string>();

  for (const file of input.changedFiles ?? []) {
    for (const { re, label } of SAFETY_CRITICAL_PATHS) {
      if (re.test(file)) signals.add(`path:${label} (${file})`);
    }
  }
  if (input.text) {
    for (const { re, label } of SAFETY_CRITICAL_KEYWORDS) {
      const m = input.text.match(re);
      if (m) signals.add(`keyword:${label} ("${m[0]}")`);
    }
  }
  return [...signals];
}
...
function lowRiskSignals(input: ClassificationInput): string[] {
  const signals = new Set<string>();
  ...
  if (files.length > 0 && files.every((f) => LOW_RISK_PATHS.some(({ re }) => re.test(f)))) {
    const labels = new Set(
      files.flatMap((f) => LOW_RISK_PATHS.filter(({ re }) => re.test(f)).map(({ label }) => label)),
    );
 ...
 */
export function classifyDeliveryBudget(input: ClassificationInput): DeliveryBudget {
  ...
  if (safetyCritical) {
    if (input.override && RANK[input.override] < RANK.HIGH && !input.allowUnsafeOverride) {
      ...
      return {
        level: 'HIGH',
        reason:
          `Safety-critical change; the requested ${input.override} override was refused because ` +
    ...
    const level: DeliveryBudgetLevel = input.override
 ...
 */
export function escalateBudget(
  budget: DeliveryBudget,
  to: DeliveryBudgetLevel,
  reason: string,
  evidence: string[],
  ...
  if (RANK[to] <= RANK[budget.level]) {
    return {
      ...
      budget,
      refusedReason:
        `Refused: ${to} does not raise the current ${budget.level} budget. ` +
  ...
  if (evidence.length === 0) {
    return {
      ...
      budget,
      refusedReason:
        'Refused: escalation requires concrete evidence (a failing command, a discovered dependency, a scan result). ' +
        'Arbitrary escalation is the same waste the delivery budget exists to prevent.',
    };
  }

  const escalation: BudgetEscalation = {
    from: budget.level,
    to,
    reason,
    evidence,
...
export function suggestEscalation(
  currentLevel: DeliveryBudgetLevel,
  observations: { failureOutput?: string; discoveredFiles?: string[] },
): { to: DeliveryBudgetLevel; reason: string; evidence: string[] } | null {
...
export function describeBudget(budget: DeliveryBudget): string {
