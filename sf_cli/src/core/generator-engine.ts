...
export interface JwtOptions {
  ...
  expiresIn?: string; // e.g., '1h', '24h', '7d', '30d'
}
...
const ENV_TEMPLATES: Record<string, EnvTemplate> = {
  api: {
    name: 'API Server',
 ...
 */
export function generateSecret(length = 64, encoding: 'hex' | 'base64' | 'base64url' = 'hex'): string {
  const bytes = randomBytes(Math.ceil(length / 2));
  if (encoding === 'base64') return bytes.toString('base64').slice(0, length);
  if (encoding === 'base64url') return bytes.toString('base64url').slice(0, length);
 ...
 */
export function generatePassword(length = 32, includeSpecial = true): string {
  let chars = PASSWORD_CHARS.lower + PASSWORD_CHARS.upper + PASSWORD_CHARS.digits;
  if (includeSpecial) chars += PASSWORD_CHARS.special;
  ...
  if (includeSpecial) ensure.push(PASSWORD_CHARS.special);
...
export function generateHash(
  input: string,
  algorithm: 'sha256' | 'sha512' | 'scrypt' = 'sha256',
): string {
  if (algorithm === 'scrypt') {
...
export function generateKeyPair(
  ...
  const result: KeyPairResult = { publicKey, privateKey, algorithm };
 ...
 */
export function generateJwt(options: JwtOptions = {}): JwtResult {
  ...
  if (options.tenant) payload.tenant = options.tenant;
  ...
  if (alg === 'HS256') {
    ...
    if (!options.secret) payload._generated_secret = secret;
    signature = base64url(
 ...
 */
export function generateTotpSecret(): { secret: string; uri: string; qrText: string } {
 ...
 */
export function getEnvTemplates(): Array<{ name: string; title: string; description: string; varCount: number }> {
 ...
 */
export function autoGenerateSecrets(projectPath: string): { generated: string[]; skipped: string[] } {
...
export function formatJwtResult(result: JwtResult): string {
  const lines = [
    'JWT Token Generated',
    LINE.repeat(60),
    '',
  ...
  if (result.keyFile) lines.push(`  Key:     ${result.keyFile}`);
  lines.push('');
...
export function formatKeyPairResult(result: KeyPairResult): string {
  const lines = [
...
export function formatAutoGenerateResult(result: { generated: string[]; skipped: string[] }): string {
  const lines = [
  ...
  if (result.generated.length > 0) {
    ...
    for (const g of result.generated) lines.push(`    \u2713 ${g}`);
  }
