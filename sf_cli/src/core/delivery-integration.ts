...
import {
  revParse, isAncestor, changedFilesBetween, workingTreeStatus,
} from './mission-git.js';
import {
  invalidateForFiles, decideValidation,
  type EvidenceQuery, type ValidationKind, type EvidenceEntry,
} from './delivery-evidence.js';
import {
  aggregateRequiredScope, scopeAtLeast, type TestScope,
} from './delivery-policy.js';
import { invalidateFactsForFiles, aggregateChangedFiles, type WorkerHandoff } from './delivery-context.js';
import type { DeliveryBudgetLevel } from './delivery-budget.js';
import { getLogger } from '../utils/logger.js';

// ── Worker verification (§11 steps 1-2) ───────────────────────────────────────
 ...
 */
export function verifyHandoff(workDir: string, handoff: WorkerHandoff): HandoffVerification {
  const problems: string[] = [];

  const base = revParse(workDir, handoff.baseCommit);
  const result = revParse(workDir, handoff.resultCommit);

  if (!base) problems.push(`Base commit ${handoff.baseCommit} cannot be resolved`);
  if (!result) problems.push(`Result commit ${handoff.resultCommit} cannot be resolved`);

  if (base && result && !isAncestor(workDir, base, result)) {
    problems.push(
      `Claimed base ${handoff.baseCommit.slice(0, 8)} is not an ancestor of ${handoff.resultCommit.slice(0, 8)} — the handoff misstates its baseline`,
    );
  }
  ...
  if (base && result) {
    ...
    if (undeclared.length > 0) {
      problems.push(
        `${undeclared.length} file(s) changed but not declared in the handoff: ${undeclared.slice(0, 5).join(', ')}`,
      );
    }
  }
...
export interface IntegrationPlan {
  ...
  deduplicated: Array<{ taskId: string; scope: TestScope; why: string }>;
...
export interface IntegrationGateInput {
  ...
  requiredValidations: Array<{ kind: ValidationKind; command: string; scope?: TestScope }>;
 ...
 */
export function planIntegrationGate(workDir: string, input: IntegrationGateInput): IntegrationPlan {
  ...
  const validations: GateValidation[] = [];
  ...
  const deduplicated = input.handoffs
    ...
    .map((h) => ({
      ...
      why:
        `Worker already proved its change at "${h.validationScope}"; the gate covers the ` +
 ...
 */
export function reasonsToRevalidate(
  workDir: string,
  ...
  const integrationTouched = handoff.changedFiles.filter((f) =>
    plan.integrationChangedFiles.includes(f),
  );
  ...
  if (!scopeAtLeast(handoff.validationScope, plan.requiredScope) && plan.requiredBudget === 'HIGH') {
    reasons.push(
      `Integration policy requires "${plan.requiredScope}" for a HIGH-risk aggregate; this worker ran "${handoff.validationScope}"`,
    );
  }
...
export function describeIntegrationPlan(plan: IntegrationPlan): string[] {
  const lines: string[] = [];
  ...
  for (const v of reuse) lines.push(`  REUSE ${v.kind}: ${v.reason}`);
  for (const v of run) lines.push(`  RUN   ${v.kind}: ${v.reason}`);
