...
import { FINDING_SCHEMA, FILE_REF_SCHEMA, type ContractMode } from './message-schemas.js';
...
export const ARCHETYPE_OUTPUT_CONTRACTS: Record<AgentArchetype, Record<string, unknown>> = {
 ...
 */
export const AGENT_OUTPUT_OVERRIDES: Record<string, Record<string, unknown>> = {
...
export function getAgentOutputContract(agentName: string): Record<string, unknown> {
 ...
 */
export function extractStructuredOutput(text: string): unknown | null {
 ...
 */
export function validateAgentResultText(agentName: string, text: string): AgentResultValidation {
  const archetype = getAgentArchetype(agentName);
...
function requiredFields(agentName: string): string[] {
  const schema = getAgentOutputContract(agentName) as { required?: unknown };
 ...
 */
export function structuredOutputInstruction(agentName: string): string {
...
export function contractedAgentNames(): string[] {
 ...
 */
export function registerAgentResultContracts(registry: MessageContractRegistry): void {
  registry.register('result:complete', { ...AGENT_RESULT_SCHEMA });
}

function formatErrors(errors: ErrorObject[] | null | undefined): string[] {
  if (!errors || errors.length === 0) return ['output failed the agent contract'];
