...
export interface RecallFilters {
  type?: string;
  minWeight?: number;
  since?: string; // "7d", "30d", or ISO date
  tags?: string[];
  limit?: number;
}
 ...
 */
export async function recallIndex(
  query: string,
  workDir: string,
...
// ── Scoring (TF-IDF inspired, matches semantic-search.sh) ──────
...
function scoreEntries(entries: KnowledgeEntry[], query: string): ScoredEntry[] {
  ...
  return entries.map((entry) => {
    ...
    for (const word of queryWords) {
      if (contentLower.includes(word)) wordMatches += 10;
    }
    ...
    const breakdown: ScoreBreakdown = { exactMatch, wordMatches, typeBonus, weightBonus, tagBonus };
...
function applyFilters(entries: KnowledgeEntry[], filters?: RecallFilters): KnowledgeEntry[] {
...
function loadAllEntries(workDir: string): KnowledgeEntry[] {
 ...
 */
export function formatPreviewResults(results: PreviewResult[]): string {
  if (results.length === 0) return 'No entries found for the given IDs.';
 ...
 */
export function formatFullResults(results: FullResult[]): string {
  if (results.length === 0) return 'No entries found for the given IDs.';
