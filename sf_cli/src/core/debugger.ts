...
import type { CDPAdapter } from './debugger-cdp.js';
...
// ── Breakpoint & inspection result types ────────────────────────────
...
export function detectRuntime(workDir: string): DebugRuntime {
  if (existsSync(join(workDir, 'tsconfig.json'))) return 'node';
  if (existsSync(join(workDir, 'package.json'))) return 'node';
  if (existsSync(join(workDir, 'requirements.txt'))) return 'python';
  if (existsSync(join(workDir, 'pyproject.toml'))) return 'python';
  if (existsSync(join(workDir, 'Cargo.toml'))) return 'lldb';
  return 'node';
}
...
export class DebugSession {
  private static activeSession: DebugSession | null = null;
  ...
  private cdp: CDPAdapter | null = null;
  private status: DebugSessionInfo['status'] = 'starting';
   ...
   */
  static async start(options: DebugStartOptions): Promise<DebugSession> {
    const log = getLogger();
    ...
    if (runtime === 'python') {
      throw new Error(
        'Python debugging requires debugpy. Install with: pip install debugpy. (Coming in Phase 2)',
      );
    }
    if (runtime === 'lldb') {
   ...
   */
  static async terminateActive(): Promise<void> {
    if (DebugSession.activeSession) {
  ...
  async setBreakpoint(file: string, line: number, condition?: string): Promise<BreakpointResult> {
    ...
    return {
      breakpointId: response.breakpointId as string,
      file,
      line,
      condition,
      verified: Array.isArray(response.locations) && (response.locations as unknown[]).length > 0,
    };
  }
   ...
   */
  async inspect(target: 'scope' | 'callstack' | string): Promise<InspectionResult> {
    this.assertAlive();
  ...
  async evaluate(expression: string): Promise<EvalResult> {
    ...
    if (response.exceptionDetails) {
      const exDetails = response.exceptionDetails as Record<string, unknown>;
      throw new Error(
        `Evaluation error: ${(exDetails.text as string) || 'unknown error'}`,
      );
    }
   ...
   */
  async step(action: 'next' | 'into' | 'out' | 'continue' | 'pause'): Promise<StepResult> {
    this.assertAlive();
   ...
   */
  async stop(): Promise<{ status: string; durationMs: number }> {
    if (this.stopped) {
  ...
  private async spawnAndConnect(options: DebugStartOptions): Promise<void> {
    const log = getLogger();
    ...
    if (options.testCommand) {
      ...
      if (!ALLOWED_TEST_RUNNERS.has(baseCommand)) {
        throw new Error(
          `Unknown test runner: ${baseCommand}. Allowed: ${[...ALLOWED_TEST_RUNNERS].join(', ')}`,
        );
      }
  ...
  private waitForDebuggerUrl(child: ChildProcess): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        if (!resolved) {
          resolved = true;
          reject(new Error(
            `Timed out waiting for debugger URL (10s). stderr: ${stderrBuffer.slice(0, 500)}`,
          ));
        }
      }, 10_000);
      ...
      child.on('exit', (code: number | null) => {
        if (!resolved) {
          ...
          clearTimeout(timeout);
          reject(new Error(
            `Debug process exited before debugger connected (code ${code}). stderr: ${stderrBuffer.slice(0, 500)}`,
          ));
        }
      });
    });
