...
// ---------------------------------------------------------------------------
// Raw Checkov JSON output schema (subset of actual output)
// ---------------------------------------------------------------------------
...
interface CheckovRawOutput {
  check_type?: string;
  results?: {
    passed_checks?: CheckovRawCheck[];
    failed_checks?: CheckovRawCheck[];
    skipped_checks?: CheckovRawCheck[];
 ...
 */
export function mapCheckovSeverity(raw: string | null | undefined): CheckovFinding['severity'] {
...
const IAC_PATTERNS: Array<{ regex: RegExp; framework: string }> = [
 ...
 */
function isCloudFormationFile(filePath: string): boolean {
  try {
 ...
 */
export function detectIaCFiles(
  dir: string,
  maxDepth = 6,
): Array<{ path: string; framework: string }> {
  const found: Array<{ path: string; framework: string }> = [];
 ...
 */
export function parseCheckovCheck(
  check: CheckovRawCheck,
  status: CheckovFinding['status'],
  targetPath: string,
): CheckovFinding {
...
export function parseCheckovOutput(
...
): { findings: CheckovFinding[]; passedCount: number; scannedFiles: number } {
  ...
  const blocks: CheckovRawOutput[] = Array.isArray(parsed)
    ? (parsed as CheckovRawOutput[])
    : [parsed as CheckovRawOutput];
  ...
  for (const block of blocks) {
    ...
    for (const check of results.failed_checks ?? []) {
      ...
      if (finding.file) scannedFilesSet.add(finding.file);
    }
 ...
 */
export function findCheckovBinary(): string | null {
...
export class CheckovScanner {
   ...
   */
  private static validateArgValue(label: string, value: string): void {
 ...
 */
export function createCheckovScanner(projectRoot: string): CheckovScanner {
 ...
 */
export function getCheckovInstallInstructions(): string {
