...
export const TEMPERATURE_DECAY = [0.7, 0.4, 0.1] as const;
...
export function repairJSON(raw: string): RepairResult {
  ...
  if (stack.length > 0) {
    ...
    for (let i = stack.length - 1; i >= 0; i--) {
      if (stack[i] === '{') { closers.push('}'); braces++; }
      else { closers.push(']'); brackets++; }
    }
    text += closers.join('');
    if (brackets > 0) fixes.push(`Closed ${brackets} unclosed bracket(s)`);
    if (braces > 0) fixes.push(`Closed ${braces} unclosed brace(s)`);
  }
 ...
 */
export function repairCodeBlocks(raw: string): RepairResult {
  const fixes: string[] = [];
  const fencePattern = /^(`{3,})\w*\s*$/gm;
  const fences: Array<{ index: number; marker: string }> = [];
 ...
 */
export function repairLLMOutput(raw: string): RepairResult {
 ...
 */
export async function withTemperatureDecay(
  opts: TemperatureDecayOptions,
): Promise<TemperatureDecayResult> {
  const log = getLogger();
  ...
  let bestRepair: TemperatureDecayResult | null = null;
  ...
  throw new Error(
    `Temperature decay exhausted after ${temps.length} attempts. No valid or repairable output produced.`,
  );
}
