...
const COMMAND_PATTERNS: Array<{ pattern: RegExp; type: CommandType }> = [
...
export function detectCommandType(command: string): CommandType {
  ...
  for (let i = segments.length - 1; i >= 0; i--) {
    ...
    for (const { pattern, type } of COMMAND_PATTERNS) {
      if (pattern.test(seg)) return type;
    }
  }
  return 'default';
}
...
export function compressGitStatus(output: string): string {
  const lines = output.split('\n');
  const result: string[] = [];

  for (const line of lines) {
    // Skip hint lines: "  (use "git ..." to ...)"
    if (/^\s*\(use "git\s/.test(line)) continue;
    // Skip empty lines
...
export function compressGitOneshot(command: string, output: string): string {
  ...
  if (/\bgit\s+push\b/.test(command)) {
    ...
    if (branch) {
      ...
      if (match) return `ok ${match[3]} ${match[1]}..${match[2]}`;
    }
  ...
  if (/\bgit\s+fetch\b/.test(command)) {
    if (!output.trim()) return 'ok (no new objects)';
...
export function compressGitLog(output: string): string {
  const lines = output.split('\n');
  ...
  for (const line of lines) {
    ...
    if (/^\s*(Author|Date|Merge):/i.test(line)) continue;
...
export function compressGitDiff(output: string, isError: boolean): string {
  if (isError) return output; // Keep full context on error
  ...
  for (const line of lines) {
    ...
    if (/^(index |mode |old mode |new mode |similarity index |rename (from|to) |copy (from|to) )/.test(line)) continue;
...
export function compressTestRunner(output: string, isError: boolean): string {
  const lines = output.split('\n');

  if (!isError) {
    // All tests passed — extract only the summary line
    const summaryLines = lines.filter((l) =>
      /Tests?:?\s+\d+|tests?\s+passed|PASS|passed|ok\s+\(/i.test(l) ||
    ...
    if (passCount > 0) return `All tests passed (${passCount} results)`;
  ...
  for (const line of lines) {
    ...
    if (inFailBlock) {
      ...
      if (blankCount >= 2) inFailBlock = false;
      continue;
    }
    ...
    if (line.trim() !== '' && !/^\s*(✓|✔|PASS|--- PASS|ok\s+\d|√)/i.test(line)) {
      ...
      if (/^running \d+ tests?$/i.test(line.trim())) continue;
      if (/^test result:/i.test(line.trim())) {
        result.push(line);
        continue;
      }
      // Skip verbose test names from passing tests
      if (/test\s+\S+\s+\.\.\.\s+ok$/i.test(line.trim())) continue;
      result.push(line);
    }
  }
...
export function compressBuildLint(output: string): string {
  const lines = output.split('\n');

  // Try to group by file. Pattern: "file:line:col: message" or "file(line,col): message"
  const fileErrors = new Map<string, Map<string, number>>();
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      const key = file.trim();
      if (!fileErrors.has(key)) fileErrors.set(key, new Map());
  ...
  if (fileErrors.size === 0) return output;
  ...
  const summaryLines = otherLines.filter((l) =>
    /error|warning|found|total|failed|\d+\s+(issue|problem)/i.test(l),
  );
  if (summaryLines.length > 0) {
    result.push('');
...
export function compressPkgInstall(output: string): string {
  const lines = output.split('\n');
  const result: string[] = [];
  ...
  for (const line of lines) {
    ...
    if (/^\[={2,}|^#{2,}\s|^\|={2,}/.test(trimmed)) continue;
    if (/^\d+%\s/.test(trimmed)) continue;
    if (/^(Downloading|Resolving|Linking|Progress|Fetching)[\s:]/i.test(trimmed)) continue;
    // Skip npm warn (keep npm ERR!)
    if (/^npm\s+warn\b/i.test(trimmed)) continue;
...
export function compressDocker(output: string): string {
  const lines = output.split('\n');
 ...
 */
function hasSignificantRepetition(output: string): boolean {
  const lines = output.split('\n');
