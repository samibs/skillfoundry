 ...
 */
export function captureSnapshots(db: Database.Database, projectId?: string): SnapshotResult {
  const log = getLogger();
  const result: SnapshotResult = { projects_captured: 0, projects_skipped: 0, errors: [] };
  ...
  for (const project of projects) {
    try {
      ...
      const existing = db.prepare(
        'SELECT id FROM kpi_snapshots WHERE project_id = ? AND snapshot_date = ?'
      ).get(project.id, today);
      ...
      const prior = db.prepare(
        'SELECT success_rate FROM kpi_snapshots WHERE project_id = ? ORDER BY snapshot_date DESC LIMIT 1'
      ).get(project.id) as { success_rate: number } | undefined;
      ...
      `).run(
        project.id,
        today,
        kpi.total_forge_runs,
        kpi.success_rate,
        kpi.gate_pass_rate,
        kpi.security_critical,
        kpi.security_high,
        0, // test_coverage — filled by future integration
        kpi.avg_cost_usd,
        trend,
      );
...
export function getSnapshots(
  ...
  `).all(projectId, days) as KpiSnapshot[];
}
...
export function getLatestSnapshots(
  db: Database.Database,
): Array<KpiSnapshot & { project_name: string }> {
  return db.prepare(`
    ...
    ORDER BY p.name
  `).all() as Array<KpiSnapshot & { project_name: string }>;
}
...
function computeMetricTrend(
  ...
  let alert = false;
  let alert_reason: string | undefined;
 ...
 */
export function computeProjectTrend(
  db: Database.Database,
  projectId: string,
  projectName: string,
  windowDays: number = 7,
  ...
  const allSnapshots = db.prepare(`
    ...
    WHERE project_id = ? AND snapshot_date >= date('now', '-' || ? || ' days')
    ORDER BY snapshot_date ASC
  `).all(projectId, windowDays * 2) as KpiSnapshot[];
  ...
  const metricDefs: Array<{ key: keyof KpiSnapshot; name: string; higherIsBetter: boolean }> = [
  ...
  const metrics = metricDefs.map((def) =>
    computeMetricTrend(
      def.name,
      recent.map((s) => s[def.key] as number),
      prior.map((s) => s[def.key] as number),
      def.higherIsBetter,
    )
  );
  ...
  let overall_direction: TrendDirection;
 ...
 */
export function generateTrendReport(db: Database.Database, windowDays: number = 7): TrendReport {
  ...
  for (const project of projects) {
    ...
    for (const metric of trend.metrics) {
      if (metric.alert && metric.alert_reason) {
        ...
        if (metric.alert_reason.includes('critical')) severity = 'critical';
        else if (metric.alert_reason.includes('dropped') || metric.alert_reason.includes('declining')) severity = 'warning';
  ...
  const successMetrics = projectTrends
    ...
    .filter((m): m is MetricTrend => m !== undefined);
...
export function forecastMetric(
  ...
  if (Math.abs(slope) < 0.001) trend_direction = 'stable';
  else if (slope > 0) trend_direction = 'improving';
  else trend_direction = 'declining';

  return { metric: metric as string, current, forecast, trend_direction };
}
 ...
 */
export function formatSnapshotResult(result: SnapshotResult): string {
  const lines = [
 ...
 */
export function formatForecast(forecasts: MetricForecast[]): string {
  const lines = [
    'KPI Forecast',
    LINE.repeat(60),
  ];
 ...
 */
export function runSnapshotCapture(dbPath: string): SnapshotResult {
