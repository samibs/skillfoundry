...
import {
  aiDir, AGENTS_DIR, readLedger, assertMissionId,
  type Ledger, type LedgerMission, type Dependency, type ExecutionStatus,
} from './mission-ledger.js';
import { analyzeCollision, type CollisionReport } from './mission-provenance.js';
...
export function assertAgentName(name: string): void {
  if (!AGENT_NAME_PATTERN.test(name)) {
    throw new Error(
      `Invalid agent name "${name}": expected 3-80 chars of [A-Za-z0-9._-] starting alphanumeric. ` +
...
export function agentPath(workDir: string, name: string): string {
...
export function readAgent(workDir: string, name: string): AgentRecord | null {
  const p = agentPath(workDir, name);
  if (!existsSync(p)) return null;
  try {
    return JSON.parse(readFileSync(p, 'utf-8')) as AgentRecord;
  } catch {
    return null;
  }
}
...
export function listAgents(workDir: string): AgentRecord[] {
  ...
  for (const file of readdirSync(dir).sort()) {
    ...
    try {
      out.push(JSON.parse(readFileSync(join(dir, file), 'utf-8')) as AgentRecord);
    } catch {
...
export function activeAgents(workDir: string): AgentRecord[] {
...
export function registerAgent(
  workDir: string,
  spec: Omit<AgentRecord, 'status' | 'startedAt' | 'endedAt'>,
  opts: { force?: boolean } = {},
): RegistrationResult {
  ...
  if (spec.mode === 'WRITE' && spec.worktree) {
    ...
    const conflict = activeAgents(workDir).find(
      (a) => a.name !== spec.name && a.mode === 'WRITE' && a.worktree && resolve(a.worktree) === target,
    );
    if (conflict) {
      blockers.push(
        `Worktree ${spec.worktree} is already owned by ACTIVE write agent "${conflict.name}" ` +
    ...
    if (existsSync(target) && !isNativeWorktree(target).registered) {
      blockers.push(
        `WORKTREE_INVALID: ${spec.worktree} is not registered in \`git worktree list --porcelain\`. ` +
...
export function releaseAgent(
  workDir: string,
  name: string,
  status: Exclude<AgentStatus, 'ACTIVE'> = 'RELEASED',
): AgentRecord | null {
...
export function sharedWorktreeViolations(workDir: string): Array<{ worktree: string; agents: string[] }> {
  const byTree = new Map<string, string[]>();

  for (const agent of activeAgents(workDir)) {
    if (agent.mode !== 'WRITE' || !agent.worktree) continue;
 ...
 */
export function buildDependencyGraph(ledger: Ledger): DependencyGraph {
  ...
  for (const [id, deps] of nodes) {
    for (const dep of deps) {
      if (!nodes.has(dep.on)) danglingEdges.push({ from: id, to: dep.on });
    }
  }
...
function findCycles(nodes: Map<string, Dependency[]>): string[][] {
  const cycles: string[][] = [];
  ...
  const visit = (id: string): void => {
    ...
    for (const dep of nodes.get(id) ?? []) {
      if (nodes.has(dep.on)) visit(dep.on);
    }
    stack.pop();
    state.set(id, 'done');
  };
...
const SATISFIED: readonly ExecutionStatus[] = ['INTEGRATED', 'VERIFIED'];
 ...
 * it is accepting.
 */
export function evaluateEligibility(ledger: Ledger, workItem: string): EligibilityVerdict {
...
export interface WavePlan {
  ...
  deferred: Array<{ workItem: string; reason: string }>;
...
export function planWave(
  workDir: string,
  opts: { maxItems?: number; only?: string[] } = {},
): WavePlan {
  ...
  const items: string[] = [];
  const deferred: WavePlan['deferred'] = [];
  ...
  for (const candidate of eligible) {
    ...
    let blockedBy: CollisionReport | null = null;

    for (const admitted of items) {
      const report = analyzeCollision(
        admitted,
        ledger.missions[admitted].execution.write_manifest,
        candidate,
        candidateManifest,
      );
      if (report.shared_files.length > 0) collisions.push(report);
 ...
 */
export function integrationOrder(ledger: Ledger, workItems: string[]): string[] {
  ...
  const visit = (id: string): void => {
    ...
    for (const dep of ledger.missions[id]?.execution.dependencies ?? []) {
      if (selected.has(dep.on)) visit(dep.on);
    }
    state.set(id, 'done');
    ordered.push(id);
  };
...
export interface OrchestrationSummary {
  ...
  shared_worktree_violations: Array<{ worktree: string; agents: string[] }>;
  unresolved_items: string[];
  dangling_dependencies: Array<{ from: string; to: string }>;
  cycles: string[][];
}

/** Summarise the orchestration plane, surfacing anything that blocks the next wave. */
export function orchestrationSummary(workDir: string): OrchestrationSummary {
  const ledger = readLedger(workDir);
  if (!ledger) {
    return {
  ...
  return {
    ...
    unresolved_items: (wave?.items ?? []).filter(
      (id) => !resolved.includes(ledger.missions[id]?.execution.status ?? 'PLANNED'),
    ),
...
export function executionOf(ledger: Ledger, workItem: string): LedgerMission['execution'] | null {
