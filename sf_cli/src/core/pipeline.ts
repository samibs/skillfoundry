 ...
 */
async function validatePrdQuality(
  prds: Array<{ file: string; content: string }>,
  model: string,
  genesisDir: string,
): Promise<void> {
  const log = getLogger();
  ...
  for (const prd of prds) {
    ...
    if (!score.pass) {
      ...
      throw new PrdQualityBlockError(
        `PRD blocked: ${prd.file} — failing dimensions: ${failingDimensions.join(', ')} (minimum ${THRESHOLD}/10)\n` +
...
export async function runPipeline(options: PipelineOptions): Promise<PipelineResult> {
  ...
  let specterStatus: PipelinePhaseStatus = 'skipped';
