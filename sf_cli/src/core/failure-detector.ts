 ...
 */
export function detectPatterns(db: Database.Database, projectId?: string): PatternReport {
  const log = getLogger();
...
function detectCrossProjectPatterns(db: Database.Database): DetectedPattern[] {
...
function detectHighFrequencyFailures(db: Database.Database, projectId?: string): DetectedPattern[] {
  ...
  for (const f of allFailures) {
    if (f.occurrences < 5) continue; // Only flag high-frequency

    const severity = f.occurrences >= 10 ? 'critical' as const
      : f.occurrences >= 5 ? 'high' as const
        : 'medium' as const;
...
function detectCategoryClusters(db: Database.Database, projectId?: string): DetectedPattern[] {
  const categories = getRecurringIssueCategories(db, projectId);
  const patterns: DetectedPattern[] = [];

  for (const cat of categories) {
    if (cat.count < 3) continue; // Only flag clusters

    const severity = cat.severity === 'CRITICAL' ? 'critical' as const
      : cat.count >= 10 ? 'high' as const
        : 'medium' as const;
...
function detectEscalatingFailures(db: Database.Database, projectId?: string): DetectedPattern[] {
  ...
  for (const row of rows) {
    ...
    if (ratePerDay < 1) continue; // Not escalating fast enough
...
function getProjectName(db: Database.Database, projectId: string): string | undefined {
