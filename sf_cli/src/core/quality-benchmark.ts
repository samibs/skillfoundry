...
const BAD_SCENARIOS: BenchmarkScenario[] = [
  // Banned patterns (T1)
...
const GOOD_SCENARIOS: BenchmarkScenario[] = [
...
export const ALL_SCENARIOS: BenchmarkScenario[] = [...BAD_SCENARIOS, ...GOOD_SCENARIOS];
 ...
 */
export function evaluateScenario(scenario: BenchmarkScenario): BenchmarkResult {
  const start = Date.now();
  const content = scenario.content;

  let actualVerdict: 'pass' | 'fail' | 'warn' = 'pass';
 ...
 */
export function runBenchmark(scenarios: BenchmarkScenario[] = ALL_SCENARIOS): BenchmarkSummary {
  const log = getLogger();
  const start = Date.now();
  ...
  for (const scenario of scenarios) {
    ...
    if (result.correct) by_category[scenario.category].correct++;
  }
