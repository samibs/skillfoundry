 ...
 */
function readEntries(filePath: string): KnowledgeEntry[] {
 ...
 */
function writeEntries(filePath: string, entries: KnowledgeEntry[]): void {
 ...
 */
export function recordRetrieval(workDir: string, entryId: string): WeightUpdate | null {
  const log = getLogger();
 ...
 */
export function runValidationUpdate(workDir: string): WeightUpdate[] {
  const log = getLogger();
  const knowledgeDir = join(workDir, KNOWLEDGE_DIR);
  ...
  for (const file of files) {
    ...
    for (const entry of entries) {
      if (!entry.reality_anchor?.test_file) continue;

      const testFile = resolve(workDir, entry.reality_anchor.test_file);
      if (!testFile.startsWith(resolve(workDir))) continue; // Path traversal protection
      const testExists = existsSync(testFile);

      if (!testExists) continue; // Test file reference might be stale
 ...
 */
export function runDecay(workDir: string, now: Date = new Date()): WeightUpdate[] {
  const log = getLogger();
  const knowledgeDir = join(workDir, KNOWLEDGE_DIR);
  if (!existsSync(knowledgeDir)) return [];
  ...
  for (const file of files) {
    ...
    for (const entry of entries) {
      ...
      if (ageWeeks < 2 || retrievals > 0) continue;
 ...
 */
export function runWeightLearning(workDir: string): WeightLearningResult {
  const log = getLogger();
  const knowledgeDir = join(workDir, KNOWLEDGE_DIR);
  const errors: string[] = [];
...
function getKnowledgeFiles(dir: string): string[] {
  try {
