...
import { VectorStore, type SearchResult } from '../core/vector-store.js';
 ...
 */
export function parseSearchArgs(args: string): MemorySearchOptions | string {
  const parts = tokenise(args);

  let query = '';
  let mode: SearchMode = 'preview';
 ...
 */
export function tokenise(args: string): string[] {
  const tokens: string[] = [];
  let current = '';
  let inQuote = '';
 ...
 * @returns Array of FormattedSearchResult.
 */
export function convertSemanticResults(
 ...
 */
export function formatIndexOutput(results: FormattedSearchResult[]): string {
...
export async function executeMemorySearch(
  ...
  try {
    const [semanticResults, provider, model] = await runSemanticSearch(
      query,
      topK,
      scope,
      session.workDir,
