...
function getDbPath(workDir: string): string {
  ...
  for (let i = 0; i < 10; i++) {
    if (ex(join(dir, '.project-registry'))) return join(dir, DEFAULT_DB_PATH);
...
export const certifyCommand: SlashCommand = {
  name: 'certify',
    ...
    switch (subcommand) {
      case 'history': {
        ...
        try {
          ...
          if (runs.length === 0) return '  No certification runs found.';
          const lines = [
...
function runAndReport(projectPath: string, flags: Record<string, string>, dbPath: string): string {
  ...
  try {
    const db = initDatabase(dbPath);
    try { insertCertificationRun(db, result); } finally { db.close(); }
  } catch { /* non-critical */ }
