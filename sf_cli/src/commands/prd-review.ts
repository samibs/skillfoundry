...
function colorScore(score: number, threshold: number): string {
  if (score >= 8) return `${GREEN}${score}/10${RESET}`;
  if (score >= threshold) return `${YELLOW}${score}/10${RESET}`;
  return `${RED}${score}/10${RESET}`;
}

function colorLabel(label: string, score: number, threshold: number): string {
  if (score >= 8) return `${GREEN}${label}${RESET}`;
  if (score >= threshold) return `${YELLOW}${label}${RESET}`;
 ...
 */
export function renderProgressBar(score: number): string {
 ...
 */
export function formatDimension(name: string, dim: PrdDimensionScore, threshold: number): string {
 ...
 */
export function getDimensionsBelow(score: PrdScore, threshold: number): [string, number][] {
 ...
 */
export function parsePrdReviewArgs(args: string): {
  filePath: string;
  json: boolean;
  threshold: number;
  verbose: boolean;
  ...
  if (thresholdFlag) {
    ...
    if (!isNaN(val) && val >= 1 && val <= 10) threshold = val;
  } else {
...
export const prdReviewCommand: SlashCommand = {
  ...
  execute: async (args: string, session: SessionContext): Promise<string> => {
    ...
    const adjustedScore: PrdScore = { ...score, pass: adjustedPass };
