 ...
 */
export function discoverSkills(workDir: string): SkillFile[] {
...
): { published: number; skipped: number; errors: string[] } {
  const log = getLogger();
...
export const publishCommand: SlashCommand = {
  ...
  execute: async (args: string, session: SessionContext): Promise<string> => {
    ...
    const lines: string[] = ['', `  ${dryRun ? '[DRY RUN] ' : ''}Publishing ${skills.length} skills`, ''];
