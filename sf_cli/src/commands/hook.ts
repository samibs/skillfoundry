...
interface HooksConfig {
  'pre-commit': { gates: string[]; fail_action: string; timeout_seconds: number };
  'pre-push': { gates: string[]; fail_action: string; timeout_seconds: number };
  options: { backup_existing: boolean };
}

function parseHooksConfig(workDir: string): HooksConfig {
...
function generateHookScript(hookType: 'pre-commit' | 'pre-push', gates: string[], failAction: string, timeoutSec: number): string {
...
export const hookCommand: SlashCommand = {
  name: 'hook',
  ...
  execute: async (args: string, session: SessionContext): Promise<string> => {
    ...
    if (action === 'install') {
      ...
      const hookTypes: Array<'pre-commit' | 'pre-push'> = ['pre-commit', 'pre-push'];

      for (const hookType of hookTypes) {
        const hookPath = join(hooksDir, hookType);
        const hookConfig = config[hookType];

        if (hookConfig.gates.length === 0) continue;
    ...
    if (action === 'uninstall') {
      ...
      if (removed.length === 0) return 'No SkillFoundry hooks found to remove.';
