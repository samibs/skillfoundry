...
import {
  assignBudget, executionPolicy, chooseTestScope, shouldRunValidation, checkCompletion,
  loadDeliverySettings, escalateBudget,
} from '../core/delivery.js';
import {
  isDeliveryBudgetLevel, describeBudget, DELIVERY_BUDGETS,
  type DeliveryBudgetLevel,
} from '../core/delivery-budget.js';
import { isTestScope, TEST_SCOPES, type TestScope } from '../core/delivery-policy.js';
import {
  loadEvidenceStore, clearEvidence, activeClaims, isValidationKind, VALIDATION_KINDS,
  type ValidationKind,
...
function parseArgs(raw: string): ParsedArgs {
  ...
  while ((m = re.exec(raw)) !== null) tokens.push(m[1] ?? m[2] ?? m[3]);
...
const ok = (s: string): string => `${GREEN}✓${RESET} ${s}`;
const bad = (s: string): string => `${RED}✗${RESET} ${s}`;
const warn = (s: string): string => `${YELLOW}⚠${RESET} ${s}`;
const head = (s: string): string => `\n  ${BOLD}${s}${RESET}\n  ${'─'.repeat(s.length)}`;

function budgetColor(level: DeliveryBudgetLevel): string {
  if (level === 'HIGH') return `${RED}${level}${RESET}`;
  if (level === 'MEDIUM') return `${YELLOW}${level}${RESET}`;
  return `${GREEN}${level}${RESET}`;
}

function requireBudgetFlag(flags: Record<string, string | true>): DeliveryBudgetLevel {
  const raw = flagString(flags, 'budget');
  if (!raw) throw new Error(`--budget is required. One of: ${DELIVERY_BUDGETS.join(', ')}`);
...
function handleStatus(workDir: string): string {
  ...
  if (summary.stale) lines.push(`  ${warn('Shared context predates the current tree — facts may be stale.')}`);
  lines.push('');
  return lines.join('\n');
}

function handleBudget(workDir: string, args: ParsedArgs): string {
  ...
  const budget = assignBudget(workDir, {
    ...
    override: override as DeliveryBudgetLevel | undefined,
  ...
  if (budget.signals.length > 0) {
    ...
    for (const sig of budget.signals.slice(0, 8)) lines.push(`    ${DIM}·${RESET} ${sig}`);
  }
...
function handlePolicy(args: ParsedArgs): string {
  const raw = args.positional[1];
  if (!raw) throw new Error(`Usage: /delivery policy <${DELIVERY_BUDGETS.join('|')}>`);
...
function handleScope(workDir: string, args: ParsedArgs): string {
  ...
  const decision = chooseTestScope(workDir, {
    ...
    override: overrideRaw as TestScope | undefined,
  ...
  if (decision.overrideRefused) lines.push(`  ${bad('Override refused — see reasons below.')}`);
  lines.push('');
  lines.push(`  ${BOLD}Why${RESET}`);
  for (const r of decision.reasons) lines.push(`    ${DIM}·${RESET} ${r}`);

  if (decision.avoided.length > 0) {
    lines.push('');
    lines.push(`  ${BOLD}Not run${RESET}`);
    for (const a of decision.avoided) lines.push(`    ${DIM}·${RESET} ${a.scope}: ${a.why}`);
  }
  lines.push('');
  return lines.join('\n');
}

function handleCheck(workDir: string, args: ParsedArgs): string {
  ...
  const decision = shouldRunValidation(
    workDir,
    {
      kind: kindRaw as ValidationKind,
      command,
      scope: scopeRaw as TestScope | undefined,
      changedFiles: flagList(args.flags, 'files'),
    },
  ...
  if (decision.evidence) {
    ...
    if (e.durationSeconds !== undefined) lines.push(`  Duration: ${e.durationSeconds}s`);
...
function handleEvidence(workDir: string, args: ParsedArgs): string {
  ...
  if (entries.length === 0) {
    ...
    for (const e of entries) {
      ...
      lines.push(
        `  ${e.kind.padEnd(20)}${e.result.padEnd(8)}${(e.scope ?? '—').padEnd(13)}${String(files || 'repo').padEnd(7)}${e.command.slice(0, 50)}`,
      );
    }
  }
  ...
  if (claims.length > 0) {
    ...
    for (const c of claims) lines.push(`    ${DIM}·${RESET} ${c.key} held by ${c.owner} since ${c.at}`);
  }
  lines.push('');
  return lines.join('\n');
}

function handleContext(workDir: string, args: ParsedArgs): string {
  ...
  if (summary.stale) lines.push(`  ${warn('Context predates the current tree.')}`);
  ...
  if (handoffs.length > 0) {
    ...
    for (const h of handoffs) {
      lines.push(
        `    ${h.taskId.padEnd(16)} ${h.budget.padEnd(7)} ${h.validationScope.padEnd(12)} ` +
...
function handleGate(workDir: string, args: ParsedArgs): string {
  ...
  for (const line of describeIntegrationPlan(plan)) lines.push(`  ${line}`);
  ...
  if (plan.untrustedHandoffs.length > 0) {
    ...
    for (const v of plan.untrustedHandoffs) {
      ...
      for (const p of v.problems) lines.push(`      ${DIM}·${RESET} ${p}`);
    }
  }
  if (plan.deduplicated.length > 0) {
    lines.push('');
    lines.push(`  ${BOLD}Worker runs not repeated${RESET}`);
    for (const d of plan.deduplicated) lines.push(`    ${ok(`${d.taskId} (${d.scope})`)} ${DIM}${d.why}${RESET}`);
  }
  lines.push('');
  return lines.join('\n');
}

function handleComplete(workDir: string, args: ParsedArgs): string {
  const budget = requireBudgetFlag(args.flags);
  ...
  const verdict = checkCompletion(workDir, {
    ...
    validationPassedAtScope: scopeRaw as TestScope | undefined,
    requiredScope: (requiredRaw as TestScope | undefined) ?? executionPolicy(budget).baseTestScope,
  ...
  if (verdict.complete) {
    ...
    for (const g of verdict.stopGuidance) lines.push(`    ${DIM}·${RESET} ${g}`);
  } else {
    lines.push(`  ${bad(`${BOLD}NOT COMPLETE${RESET}`)}`);
    for (const o of verdict.outstanding) lines.push(`    ${RED}·${RESET} ${o}`);
  }
  lines.push('');
  return lines.join('\n');
}

function handleEfficiency(workDir: string, args: ParsedArgs): string {
  const mission = flagString(args.flags, 'mission') ?? 'default';
  const report = buildEfficiencyReport(workDir, mission);
  if (flagBool(args.flags, 'json')) return JSON.stringify(report, null, 2);

  const lines = [head('Delivery efficiency')];
  for (const line of formatEfficiencyReport(report)) lines.push(`  ${line}`);
  lines.push('');
  return lines.join('\n');
}

function handleEscalate(workDir: string, args: ParsedArgs): string {
  const from = requireBudgetFlag(args.flags);
  const toRaw = flagString(args.flags, 'to');
  if (!toRaw) throw new Error('Usage: /delivery escalate --budget LOW --to MEDIUM --reason "..." --evidence "..."');
  ...
  const result = escalateBudget(
    { ...current, level: from },
    to,
    flagString(args.flags, 'reason') ?? '',
    flagList(args.flags, 'evidence'),
  );
