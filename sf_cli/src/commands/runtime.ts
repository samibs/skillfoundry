 ...
 */
function renderStatusTable(status: PoolStatus, messageCount: number, uptimeSecs: number): string {
  const now = Date.now();
  const lines: string[] = [];
  ...
  lines.push(
    `  Concurrency: ${status.running} / ${status.maxConcurrency} active │ ` +
 ...
 */
export function setActivePool(pool: AgentPool | null): void {
  activePool = pool;
}
 ...
 */
export function getActivePool(): AgentPool | null {
  return activePool;
}
