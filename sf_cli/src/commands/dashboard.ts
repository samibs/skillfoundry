...
import type { KpiSnapshot } from '../core/kpi-engine.js';
import {
  scanForRemediations,
  applyRemediation,
...
function parseArgs(args: string): { subcommand: string; positional: string[]; flags: Record<string, string> } {
...
function withDb(dbPath: string, fn: (db: Database.Database) => string): string {
...
function formatOverview(dbPath: string): string {
...
function formatStatus(dbPath: string, projectName: string): string {
  return withDb(dbPath, (db) => {
    const detail = getProjectDetail(db, projectName);
    if (!detail) return `  Project "${projectName}" not found. Run /dashboard sync first.`;
...
function formatFailures(dbPath: string, flags: Record<string, string>): string {
  return withDb(dbPath, (db) => {
...
function formatTop(dbPath: string, flags: Record<string, string>): string {
  const metric = (flags.by || 'events') as 'events' | 'failures' | 'cost' | 'perf';
...
function formatKpi(dbPath: string, flags: Record<string, string>): string {
  return withDb(dbPath, (db) => {
    ...
    if (projectName) {
      const detail = getProjectDetail(db, projectName);
      if (!detail) return `  Project "${projectName}" not found.`;
...
function formatHealth(dbPath: string): string {
  return withDb(dbPath, (db) => {
  ...
  execute: async (args: string, session: SessionContext): Promise<string> => {
    ...
    switch (subcommand) {
      ...
      case 'status': {
        ...
        if (!projectName) return '  Usage: /dashboard status <project-name>';
        ...
        if (jsonOutput) {
          return withDb(dbPath, (db) => {
            ...
            if (flags.project) {
              ...
              if (!detail) return JSON.stringify({ error: 'project not found' });
              projectId = detail.project.id;
            }
            const kpi = computeProjectKpis(db, projectId);
      ...
      case 'import': {
        ...
        return withDb(dbPath, (db) => {
          if (importPath) {
            const detail = getProjectDetail(db, importPath);
            if (!detail) return `  Project "${importPath}" not found. Run /dashboard sync first.`;
            const res = scanProjectSessions(db, detail.project.id, detail.project.path);
      ...
      case 'patterns': {
        ...
        return withDb(dbPath, (db) => {
          ...
          if (flags.project) {
            const detail = getProjectDetail(db, flags.project);
            if (!detail) return `  Project "${flags.project}" not found.`;
            projectId = detail.project.id;
          }
      ...
      case 'forecast': {
        const projectName = positional[0] || flags.project;
        if (!projectName) return '  Usage: /dashboard forecast <project> [--metric success_rate|gate_pass_rate|avg_cost_usd]';

        return withDb(dbPath, (db) => {
          const detail = getProjectDetail(db, projectName);
          if (!detail) return `  Project "${projectName}" not found.`;

          const metrics: Array<keyof KpiSnapshot> = ['success_rate', 'gate_pass_rate', 'avg_cost_usd'];
          if (flags.metric) {
            const forecasts = [forecastMetric(db, detail.project.id, flags.metric as keyof KpiSnapshot)];
      ...
      case 'remediate': {
        ...
        switch (remAction) {
          ...
          case 'list': {
            if (jsonOutput) {
              return withDb(dbPath, (db) => {
                ...
                if (flags.project) {
                  const detail = getProjectDetail(db, flags.project);
                  if (detail) projectId = detail.project.id;
                }
          ...
          case 'apply': {
            ...
            if (!remId) return '  Usage: /dashboard remediate apply <remediation-id> [--action start|complete|fail|skip]';
            const action = (flags.action || 'start') as 'start' | 'complete' | 'fail' | 'skip';
            return withDb(dbPath, (db) => {
