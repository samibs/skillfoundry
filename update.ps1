...
function Update-Project {
    ...
    $sfDir = Join-Path $ProjectDir ".skillfoundry"
    $configToml = Join-Path $sfDir "config.toml"
...
# Generated by update.ps1 v$FrameworkVersion
...
log_level = "info"
"@
        Set-Content -Path $configToml -Value $tomlContent -Encoding UTF8
        Write-ColorOutput "  [OK] Created .skillfoundry/config.toml (enables VS Code extension)" "Green"
    } else {
        (Get-Content $configToml) -replace '^version = .*', "version = `"$FrameworkVersion`"" | Set-Content $configToml
    }
