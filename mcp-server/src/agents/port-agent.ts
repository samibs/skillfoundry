 ...
 */
export async function assignPort(appName: string): Promise<PortResult> {
        ...
        if (line.includes(appName)) {
          ...
          if (portMatch) {
            ...
            return {
              ...
              method: "portman",
              appName,
              conflict: inUse ? `Port ${port} already in use by another process` : null,
...
export async function checkPort(
  port: number
): Promise<{ inUse: boolean; pid: string | null; process: string | null }> {
  ...
  if (lines.length > 1) {
    ...
    return { inUse: true, pid: parts[1] || null, process: parts[0] || null };
  }

  return { inUse: true, pid: null, process: null };
}
