/**
 * PlaywrightAgent execution logic — browser-based auth flow verification.
 */
 ...
 * This is NOT an LLM opinion — it is deterministic tool output.
 */
export async function verifyAuthFlow(
  ...
  try {
    ...
    page.on("requestfailed", (req) => {
      // Log network failures for debugging (captured in evidence)
      void req.failure()?.errorText;
    });
