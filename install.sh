...
mkdir -p "$TARGET_DIR/.skillfoundry"

# Create config.toml (enables VS Code extension activation + framework config)
if [ ! -f "$TARGET_DIR/.skillfoundry/config.toml" ]; then
    cat > "$TARGET_DIR/.skillfoundry/config.toml" << TOML_EOF
# SkillFoundry Configuration
# Generated by install.sh v${FRAMEWORK_VERSION}
...
[platforms]
installed = [$(printf '"%s"' "${PLATFORMS[0]}"; for p in "${PLATFORMS[@]:1}"; do printf ', "%s"' "$p"; done)]
...
log_level = "info"
TOML_EOF
    echo -e "${GREEN}  ✓ .skillfoundry/config.toml created${NC}"
fi
