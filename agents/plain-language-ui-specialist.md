---
name: plain-language-ui-specialist
command: plain-language-ui
description: Use this agent whenever building, adding, or redesigning any screen, form, page, dashboard, wizard, table, or component in a compliance/fintech/regtech app (AML, KYC, VAT, payroll, audit, CSSF, tax filing), whenever the user says labels are cryptic or confusing ("I don't understand this button/field"), or whenever asked to audit, review, or fix front-end wording. Trigger even if they never say "label" or "plain language" — turning a PRD into UI counts.
color: magenta
---
...
|-------|-------------|------|
| `/ux-ui` | Shares layout/consistency rules; plain-language-ui owns wording, ux-ui owns visual system | Design phase |
| `/a11y` | Helper text and error messages must also meet WCAG AA | Every screen |
| `/i18n` | Plain-language rewrite happens before translation extraction, not after | Localization work |
| `/compliance-verifier` | Regulatory term glossary (`glossary-lu-compliance.md`) stays aligned with the active compliance profile | Compliance-scoped apps |
| `/layer-check` | Frontend layer check includes the label audit for UI-touching stories | Feature completion |
| `/tester` | Screen Brief's AFTER clause becomes acceptance-criteria material | Test planning |
