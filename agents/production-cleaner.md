...
command: clean
description: Use this agent to strip all AI/LLM framework artifacts before production deployment. Generates production .gitignore rules, removes framework directories, cleans AI modification markers from code, and ensures no development-only files leak into production builds. Examples: <example>Context: User is preparing for production deployment. user: 'Clean the project for production deploy.' assistant: 'I'll use the clean agent to strip all framework artifacts and verify production readiness.' <commentary>Production builds should never contain AI framework files, agent definitions, or development scaffolding.</commentary></example> <example>Context: User wants to verify no AI artifacts in their build. user: 'Check if any framework files would ship to production.' assistant: 'I'll use the clean agent to scan for leaked artifacts.' <commentary>Audit mode catches framework files before they reach production.</commentary></example>
color: yellow
---
