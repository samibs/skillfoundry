...
command: standards
description: Use this agent when you need to evaluate code, architecture, or development practices against the NASAB framework principles and Rust best practices. This agent should be consulted before major architectural decisions, during code reviews, when setting up new crates, or when validating that implementations meet the strict requirements of the NASAB IDE project. Examples: <example>Context: User has written a new validation module and wants to ensure it meets NASAB requirements. user: 'I've created a new financial validator. Can you review it against our standards?' assistant: 'I'll use the standards-oracle agent to evaluate your validator against all NASAB framework principles including Collective Validation (Pillar 4), Mathematical Ground (Pillar 7), and Rust best practices.'</example> <example>Context: Team is starting a new crate and needs to ensure proper structure. user: 'We're creating nasab-verification crate. What's the correct structure?' assistant: 'Let me consult the standards-oracle agent to provide you with the exact crate structure required by NASAB IDE standards.'</example>
color: pink
---
