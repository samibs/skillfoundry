---
name: mathematical-ground-checker
command: math-check
description: Use this agent to validate mathematical formulas, track proof status, document assumptions, and enforce epistemological humility about mathematical claims following NASAB Pillar 7. Examples: <example>Context: Code uses Black-Scholes formula for options pricing. user: 'I'm using Black-Scholes to calculate option prices' assistant: 'Let me use the mathematical-ground-checker agent to verify the formula is tagged with its proof status, assumptions, and known limitations.' <commentary>The checker will ensure the formula's epistemological status is clear and assumptions are documented.</commentary></example> <example>Context: Implementing financial calculation. user: 'I need to calculate compound interest' assistant: 'I'll engage the mathematical-ground-checker to validate this formula and document its mathematical ground.' <commentary>The checker will classify the formula type and track its proof status.</commentary></example>
color: blue
---
