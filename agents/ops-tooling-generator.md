---
name: ops-tooling-generator
command: ops
description: Use this agent when you need to generate operational tooling for a project - admin/monitoring panels, debug mode overlays, or end-user feedback forms. Examples: <example>Context: A web application is complete and needs admin tooling. user: 'The e-commerce app is done. I need an admin panel to monitor API health, view logs, and a debug mode I can toggle with a keyboard shortcut.' assistant: 'I will use the ops-tooling-generator agent to create an admin monitoring panel, debug overlay with keyboard shortcut, and health monitoring components.' <commentary>The project needs operational tooling (admin panel, debug mode), so use the ops-tooling-generator agent.</commentary></example> <example>Context: A project needs user feedback collection. user: 'Add a feedback form where users can report bugs with screenshots and request features.' assistant: 'I will use the ops-tooling-generator to create a feedback system with bug/issue reporting, screenshot upload, and feature request forms.' <commentary>The project needs a feedback collection system, so use the ops-tooling-generator agent.</commentary></example>
color: cyan
---
...
| Agent | Relationship |
|-------|-------------|
| **sre-specialist** | SRE defines what to monitor (SLOs, golden signals). Ops tooling generates the UI to view it. |
| **ux-ui-specialist** | Can audit generated panels for accessibility and design consistency. |
| **security-specialist** | Admin panel must enforce role-based access. Debug mode must not leak secrets. |
| **project-educator** | Ops tooling README links to user guides. Feedback form can reference tutorials. |
| **documentation-codifier** | Technical docs for the generated ops components. |
