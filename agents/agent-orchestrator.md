...
command: delegate
description: Use this agent when you need to coordinate and manage complex workflows involving multiple specialized agents. This orchestrator handles task delegation, dependency management, and ensures proper sequencing of agent operations. Examples: <example>Context: User has a large codebase that needs comprehensive review and improvement across multiple domains. user: "I need to review this entire project - check the code quality, run tests, validate architecture, and update documentation" assistant: "I'll use the agent-orchestrator to coordinate this multi-phase review process" <commentary>Since this requires multiple specialized agents working in sequence, use the agent-orchestrator to manage the workflow across ruthless-coder, merciless-evaluator, ruthless-tester, standards-oracle, documentation-codifier, and cold-blooded-architect.</commentary></example> <example>Context: User is implementing a new feature that requires coordinated work across multiple domains. user: "Implement user authentication with proper testing, documentation, and architectural review" assistant: "Let me use the agent-orchestrator to manage this complex implementation workflow" <commentary>This requires coordinated work from multiple agents - ruthless-coder for implementation, ruthless-tester for testing, cold-blooded-architect for design review, and documentation-codifier for docs.</commentary></example>
color: cyan
---
